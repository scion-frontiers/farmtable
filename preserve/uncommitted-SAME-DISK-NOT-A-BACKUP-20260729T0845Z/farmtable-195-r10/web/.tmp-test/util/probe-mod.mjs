...
function spoofingAttributes() {
    ...
    check('DOM-clobbering id/name attributes stripped', () => {
        const out = renderMarkdown('<a name="body">alpha</a><a id="body">bravo</a><p id="children">charlie</p>' +
...
function scriptExecution() {
    ...
    check('fixture: the inline-handler assertion fires on HTML that carries a handler', () => {
        ...
        try {
            ...
            missed.push(`FALSE POSITIVE on handler-free HTML: ${e.message}`);
        }
        if (missed.length > 0) {
    ...
    check('unknown URL schemes are dropped', () => {
        assertNotContains(renderMarkdown('<a href="evilproto:payload">x</a>'), 'evilproto:', 'unknown protocol survived — has ALLOW_UNKNOWN_PROTOCOLS been enabled?');
    });
...
function inputContract() {
    ...
    check('renderMarkdown accepts exactly one parameter', () => {
        const violation = renderMarkdownArityViolation(readFileSync(join(findWebRoot(), 'src', 'util', 'markdown.ts'), 'utf8'));
        if (violation !== null)
            throw new Error(`src/util/markdown.ts: ${violation}`);
        ...
        assertEqual(String(renderMarkdown.length), '1', 'the compiled renderMarkdown does not take exactly one argument, even though the ' +
    ...
    check('fixture: the arity pin catches every known widening and rejects nothing correct', () => {
        ...
        for (const { label, replace } of ARITY_LEGITIMATE) {
            const violation = renderMarkdownArityViolation(ARITY_SOUND_SOURCE.replace(ARITY_DECL, replace));
            if (violation !== null) {
        ...
        const unclosed = renderMarkdownArityViolation('export function renderMarkdown(md: string, opts = {}: string {\n  return md;\n}');
        if (unclosed === null) {
            problems.push('SURVIVED: an unterminated parameter list is not reported — the round-8 fallback ' +
    ...
    check('fixture: every structural scanner reports from the original, not the blinded view', () => {
        const REPORT_FROM_ORIGINAL = [
            {
                label: 'balancedDeclarationParameterLists slices the parameter list from the original',
                produced: balancedDeclarationParameterLists('export function renderMarkdown(md: string | `)`): string {').lists.join(' ~ '),
                mustContain: '`)`',
            },
            {
        ...
        for (const { label, produced, mustContain } of REPORT_FROM_ORIGINAL) {
            if (!produced.includes(mustContain)) {
                problems.push(`${label}: expected the result to quote ${mustContain}, got ${JSON.stringify(produced)}` +
        ...
        if (sinkArgumentIsSanitized('renderMarkdown(this.body, /* opts */)')) {
            problems.push('the extra-argument arm is reading the blinded view: a second argument consisting only ' +
                'of a comment blanks to spaces and the call was accepted');
        }
        if (sinkArgumentIsSanitized('renderMarkdown(this.body) /* trailing */')) {
            problems.push('the trailing-text arm is reading the blinded view: trailing text consisting only of a ' +
        ...
        if (problems.length > 0) {
            throw new Error('the decide-blinded / report-raw split is broken: ' + problems.join(' | '));
        }
    });
    ...
    check('renderMarkdown does not throw on non-string input', () => {
        for (const bad of [undefined, null, 42, {}, []]) {
            ...
            let out;
            try {
                out = renderMarkdown(bad);
            }
            catch (err) {
                throw new Error(`renderMarkdown threw on ${label} input (${err.message}) — a throw inside ` +
...
// ---------------------------------------------------------------------------
function sharedMarkedSingleton() {
    ...
    check('renderMarkdown does not use the shared marked singleton', () => {
        marked.use({
            renderer: {
                checkbox: () => '<img src=x onerror=alert(1)>',
            },
        });
        ...
        assertContains(poisoned, 'onerror', 'the hostile renderer never took effect on the singleton, so the rest of this check is vacuous');
        const out = renderMarkdown('- [x] done\n');
        assertNotContains(out, 'onerror', 'a renderer installed on the shared marked singleton reached renderMarkdown');
        assertNotContains(out, '<img', 'a renderer installed on the shared marked singleton reached renderMarkdown');
...
// ---------------------------------------------------------------------------
function privateDOMPurifyInstance() {
    check('renderMarkdown does not use the process-global DOMPurify singleton', () => {
        ...
        assertContains(poisoned, 'onerror', 'setConfig never took effect on the DOMPurify singleton, so the rest of this check is vacuous');
        assertContains(poisoned, '<script', 'setConfig never took effect on the DOMPurify singleton, so the rest of this check is vacuous');
        const out = renderMarkdown(payload);
        assertNotContains(out, 'onerror', 'a configuration set on the process-global DOMPurify singleton reached renderMarkdown — ' +
        ...
        assertNotContains(out, '<script', 'a configuration set on the process-global DOMPurify singleton reached renderMarkdown — ' +
 ...
 */
function literalBlindView(code) {
...
const ARITY_EVASIONS = [
    ...
    },
    {
        ...
        replace: 'export function renderMarkdown(md: string, { inline }: { inline?: boolean } = {}): string {',
    },
    {
    ...
    },
    {
        ...
        replace: 'export function renderMarkdown(md: string | ((x: string) => string), ' +
    ...
    },
    {
        ...
        replace: 'export function renderMarkdown(md: (x: string) => string, ' +
            'opts: { inline?: boolean } = {}): string {',
    },
    ...
    // keyed on the union syntax cannot pass, and q is the CROSS-AXIS control.
    {
        ...
        replace: 'export function renderMarkdown(md: string | `)`, ' +
            'opts: { inline?: boolean } = {}): string {',
    },
    {
...
const ARITY_LEGITIMATE = [
    ...
    },
    {
        ...
        replace: '// Do not add an options parameter: renderMarkdown(md, opts) is a configuration\n' +
            '// channel into the sanitizer. See the guard in markdown.test.ts.\n' +
            ARITY_DECL,
    },
    {
        label: 'a string literal naming a two-parameter form',
        replace: "export const REJECTED = 'renderMarkdown(md, opts) is not a supported signature';\n" +
            ARITY_DECL,
    },
...
function balancedDeclarationParameterLists(code) {
    ...
    while (m !== null) {
        ...
        if (depth !== 0)
            return { lists: out, unterminated: true };
 ...
 */
function renderMarkdownArityViolation(src) {
    ...
    if (unterminated) {
        return ("renderMarkdown's parameter list is not closed in the scanned view. Either the source " +
    ...
    if (decls.length === 0) {
        return ('could not find an `export function renderMarkdown(…)` declaration. Either it was ' +
    ...
    if (decls.length !== 1) {
        return (`expected exactly one renderMarkdown declaration, found ${decls.length} — an overload ` +
    ...
    if (params.length !== 1) {
        return (`renderMarkdown declares ${params.length} parameters: (${decls[0].trim()}) — a second ` +
    ...
    if (only.startsWith('{') || only.startsWith('[')) {
        return (`renderMarkdown's sole parameter is a destructuring pattern: (${only}) — that is a ` +
    ...
    if (hasTopLevelDefault(only)) {
        return (`renderMarkdown's sole parameter has a default: (${only}) — harmless in itself, but it ` +
...
function directiveIndirectionOffenders(rel, code) {
    ...
    for (const { name, module } of RAW_DIRECTIVES) {
        ...
        const renamed = new RegExp(`(?:import|export)\\s*(?:type\\s+)?\\{[^}]*\\b${name}\\s+as\\s+`).exec(code);
        if (renamed) {
            offenders.push(`${rel}:${lineOf(code, renamed.index)}: ${name} renamed with 'as' in an ` +
                'import/export clause');
        }
        const namespaced = new RegExp(`import\\s*\\*\\s*as\\s+\\w+\\s+from\\s*['"][^'"]*${mod}['"]`).exec(code);
        if (namespaced) {
            offenders.push(`${rel}:${lineOf(code, namespaced.index)}: ${module} imported as a namespace`);
        }
 ...
 */
function sourceFileCountViolation(found) {
    if (found === EXPECTED_SOURCE_FILES)
        return null;
    return (`expected to scan exactly ${EXPECTED_SOURCE_FILES} source files, found ${found} — ` +
...
function sinkCountViolation(found) {
    if (found === REQUIRED_SINKS.length)
        return null;
    return (`expected exactly ${REQUIRED_SINKS.length} unsafeHTML call sites, found ${found} — ` +
 ...
 */
function fixtureTableViolation(name, table, expected) {
    if (table.length === expected)
        return null;
    return (`${name} has ${table.length} entries, expected ${expected}. If you ADDED a fixture, ` +
 ...
 */
function escapeInCodeOffenders(rel, codeNoStrings) {
 ...
 */
function dynamicImportSpecifierOffenders(rel, code) {
    const out = [];
    for (const arg of callArguments(code, 'import')) {
        if (!/^\s*['"][^'"]*['"]\s*$/.test(arg)) {
            out.push(`${rel}:${lineOf(code, code.indexOf(arg))}: import(${arg.trim().slice(0, 60)}) — a ` +
 ...
 */
function requiredSinkScopeViolation(found) {
    if (found === EXPECTED_REQUIRED_SINKS)
        return null;
    return (`mechanism (a) is closed over ${found} file(s), but this suite claims ${EXPECTED_REQUIRED_SINKS} — ` +
...
function runTreeWide(entries, predicate) {
    const offenders = [];
    let visited = 0;
...
function scanTreeWide(entries, probe, predicate) {
    const real = runTreeWide(entries, predicate);
    const control = runTreeWide([...entries, { rel: TREE_WIDE_PROBE_REL, view: probe }], predicate);
    return {
        offenders: real.offenders,
        probed: control.offenders,
        visited: real.visited,
        blankViews: entries.filter((e) => e.view.trim() === '').map((e) => e.rel),
        canaries: entries.filter((e) => e.rel === TREE_WIDE_VIEW_CANARY_REL && e.view.includes(TREE_WIDE_VIEW_CANARY_TEXT)).length,
    };
}
/**
 ...
 */
function treeWideScanViolation(rule, scan) {
    if (scan.blankViews.length > 0) {
        return (`${rule}: ${scan.blankViews.length} of its tree-wide entries were handed to the loop ` +
    ...
    if (scan.canaries !== 1) {
        return (`${rule}: ${scan.canaries} of its tree-wide entries were named ` +
    ...
    if (scan.visited !== EXPECTED_SOURCE_FILES) {
        return (`${rule}: its tree-wide loop visited ${scan.visited} entr(ies), not ` +
    ...
    if (planted.length !== 1) {
        return (`${rule}: the planted ${TREE_WIDE_PROBE_REL} offender produced ${planted.length} ` +
...
const treeWideControlsRun = new Set();
function assertTreeWideScanSound(rule, scan) {
...
function sinkBinding() {
    ...
    check('the sanitizer exclusively owns its own dependencies', () => {
        ...
        const scan = scanTreeWide(entries, probe, (rel, view) => sanitizerOwnershipViolations(rel, view, scannedRel));
    ...
    check('tripwire: no file reaches a raw-HTML directive under another name', () => {
        const scan = scanTreeWide(scanned.map(({ rel, withStrings }) => ({ rel, view: withStrings })), 'const rawHtml = unsafeHTML;', directiveIndirectionOffenders);
    ...
    check('tripwire: no file spells an identifier with a unicode or hex escape', () => {
        const scan = scanTreeWide(scanned.map(({ rel, codeNoStrings }) => ({ rel, view: codeNoStrings })), 'const rawHtml = \\u0075nsafeHTML;', escapeInCodeOffenders);
        if (scan.offenders.length > 0) {
    ...
    check('tripwire: every dynamic import specifier is a plain quoted literal', () => {
        const scan = scanTreeWide(scanned.map(({ rel, withStrings }) => ({ rel, view: withStrings })), "const m = await import('dompur' + 'ify');", dynamicImportSpecifierOffenders);
        if (scan.offenders.length > 0) {
    ...
    check('tripwire: no listed raw-HTML sink other than unsafeHTML is present', () => {
        const scan = scanTreeWide(scanned.map(({ rel, withStrings }) => ({ rel, view: withStrings })), 'document.write(body);', (rel, view) => BANNED_SINKS.filter(({ pattern }) => pattern.test(view)).map(({ name }) => `${rel} (${name})`));
        const offenders = scan.offenders;
        if (offenders.length > 0) {
    ...
    check('fixture: legitimate source does not trip the raw-directive tripwire', () => {
        ...
        for (const fixture of ESCAPE_LEGITIMATE) {
            ...
            offenders.push(...escapeInCodeOffenders('<fixture>', codeNoStrings).map((o) => `${o} :: ${fixture}`));
        }
        if (offenders.length > 0) {
    ...
    check('fixture: every banned raw-HTML sink form is actually detected', () => {
        ...
        for (const { name, pattern } of BANNED_SINKS) {
            if (!views.some(({ code }) => pattern.test(code))) {
                problems.push(`no positive exercises the '${name}' pattern — it is unfixtured detection logic, ` +
    ...
    check('fixture: the tree-wide count pins fire on a changed count', () => {
        ...
        if (perturbations !== EXPECTED_COUNT_PIN_PERTURBATIONS) {
            missed.push(`only ${perturbations} of ${EXPECTED_COUNT_PIN_PERTURBATIONS} (pin, delta) perturbations ` +
        ...
        for (const fixture of DYNAMIC_IMPORT_EVASIONS) {
            ...
            for (const offender of found) {
                ...
                if (quoted === null) {
                    ...
                    problems.push(`offender quotes text that is not in the source: ${JSON.stringify(quoted[1])} ` +
    ...
    check('fixture: the tree-wide vacuity control fires on every unsound scan', () => {
        ...
        try {
            ...
            missed.push(`assertTreeWideScanSound REJECTED a sound scan: ${e.message}`);
        }
    ...
    check('fixture: the sink-binding rules accept a correct sink file', () => {
        ...
        for (const { label, call } of SINK_CALL_LEGITIMATE) {
            const mutated = SOUND_SINK_FILE.replace('${unsafeHTML(renderMarkdown(this.body))}', call);
            const bad = sinkBindingViolations(FIXTURE_REL, mutated, scannedRel);
            if (bad.length > 0)
                rejected.push(`FALSE POSITIVE: ${label} — ${bad.join(' | ')}`);
        }
        if (rejected.length > 0) {
            throw new Error(`a correct sanitizer call was rejected at the sink — a guard that rejects correct ` +
        ...
        if (asiViolations.length > 0) {
            throw new Error('a correct sink file with ASI-style imports was rejected — a guard that rejects ' +
    ...
    check('fixture: a string literal naming the sink identifiers is not a violation', () => {
        const STRING_BLANKING_CONTROLS = [
            {
        ...
        if (problems.length > 0) {
            throw new Error('string blanking regressed — the guard now rejects a correct sink file: ' +
                problems.join(' | '));
        }
    });
...
function dependencyPolicy() {
    const pkg = JSON.parse(readFileSync(join(findWebRoot(), 'package.json'), 'utf8'));
    ...
    check('dompurify declares a floor equal to the advisory line', () => {
        ...
        if (range !== '^3.4.12') {
            throw new Error(`dompurify is declared as ${JSON.stringify(range)}, expected "^3.4.12". ` +
    ...
    check('sunset clause: #204 tooling absent, tokenizer subset still earns its place', () => {
        const eslintDeps = Object.keys(deps).filter((d) => d.startsWith('@typescript-eslint/') || d === 'typescript-eslint');
        if (eslintDeps.length > 0) {
            throw new Error(`typescript-eslint tooling is now declared (${eslintDeps.join(', ')}), which is the ` +
...
function run() {
    ...
    if (missingControls.length > 0) {
        failures.push(`tree-wide vacuity control(s) never ran or never threw: ${missingControls.join(', ')}. ` +
    ...
    if (checks !== EXPECTED_CHECKS) {
        failures.push(`check total pinned: expected ${EXPECTED_CHECKS} checks to run ` +
    ...
    if (failures.length === 0 && assertions !== EXPECTED_ASSERTIONS) {
        failures.push(`assertion total pinned: expected ${EXPECTED_ASSERTIONS} assertions to run, ${assertions} ` +
...
export { sinkArgumentIsSanitized, renderMarkdownArityViolation };
