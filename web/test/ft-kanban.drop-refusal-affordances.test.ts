...
/**
 * The single anchor for the refusal wording.
 *
 * Everything else in the suite derives its expectation from `DROP_REFUSAL`, so
 * a reword propagates silently everywhere. That is the right trade — derived
 * expectations cannot drift — but it leaves the user-visible copy itself
 * unpinned. These literals are the specification, and this is deliberately the
 * ONLY place they appear as literals: change the copy and exactly one test
 * fails, which is a decision point rather than an accident.
 */
