...
- **A leg cloned into `/tmp` IS NOT ON THE SAME FILESYSTEM AS THE HOST, and a host-side sweep
  ...
  **The direction is not symmetric:** the leg can read the host, the host cannot read the leg, so
  ...
  For this leg the answer was a real zero: 491 + 489 unique SHAs across both overlay clones
  (refs + reflogs + fsck unreachable), **0 absent** from canonical, fabricated-SHA control taking
  the absent branch in the same invocation.
