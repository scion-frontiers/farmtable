...
concurrently, and higher-numbered groups depend on lower groups completing first.`,
	...
	cmd.Flags().StringVar(&iapAudience, "iap-audience", "", "IAP OAuth client ID; when set, auto-mints an OIDC identity token for IAP (or IAP_AUDIENCE env)")
...
func mintIAPToken(audience string) (string, error) {
	...
	// This works with user credentials (gcloud auth login) and
