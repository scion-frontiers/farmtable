...
type TaskQuery struct {
	...
	withSourceRelationships *RelationshipQuery
	withTargetRelationships *RelationshipQuery
...
func (_q *TaskQuery) QueryCollection() *CollectionQuery {
...
func (_q *TaskQuery) QueryChanges() *ChangeQuery {
...
func (_q *TaskQuery) QuerySourceRelationships() *RelationshipQuery {
...
func (_q *TaskQuery) QueryTargetRelationships() *RelationshipQuery {
	query := (&RelationshipClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
...
func (_q *TaskQuery) WithCollection(opts ...func(*CollectionQuery)) *TaskQuery {
	query := (&CollectionClient{config: _q.config}).Query()
...
func (_q *TaskQuery) WithChanges(opts ...func(*ChangeQuery)) *TaskQuery {
	query := (&ChangeClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *TaskQuery) WithSourceRelationships(opts ...func(*RelationshipQuery)) *TaskQuery {
	query := (&RelationshipClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *TaskQuery) WithTargetRelationships(opts ...func(*RelationshipQuery)) *TaskQuery {
	query := (&RelationshipClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *TaskQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Task, error) {
	...
	if query := _q.withCollection; query != nil {
		if err := _q.loadCollection(ctx, query, nodes, nil,
			func(n *Task, e *Collection) { n.Edges.Collection = e }); err != nil {
			return nil, err
		}
	}
	...
	if query := _q.withChanges; query != nil {
		if err := _q.loadChanges(ctx, query, nodes,
			func(n *Task) { n.Edges.Changes = []*Change{} },
			func(n *Task, e *Change) { n.Edges.Changes = append(n.Edges.Changes, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withSourceRelationships; query != nil {
		if err := _q.loadSourceRelationships(ctx, query, nodes,
			func(n *Task) { n.Edges.SourceRelationships = []*Relationship{} },
			func(n *Task, e *Relationship) { n.Edges.SourceRelationships = append(n.Edges.SourceRelationships, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withTargetRelationships; query != nil {
		if err := _q.loadTargetRelationships(ctx, query, nodes,
			func(n *Task) { n.Edges.TargetRelationships = []*Relationship{} },
			func(n *Task, e *Relationship) { n.Edges.TargetRelationships = append(n.Edges.TargetRelationships, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *TaskQuery) loadCollection(ctx context.Context, query *CollectionQuery, nodes []*Task, init func(*Task), assign func(*Task, *Collection)) error {
...
func (_q *TaskQuery) loadChanges(ctx context.Context, query *ChangeQuery, nodes []*Task, init func(*Task), assign func(*Task, *Change)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Task)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(change.FieldTaskID)
	}
	query.Where(predicate.Change(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(task.ChangesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *TaskQuery) loadSourceRelationships(ctx context.Context, query *RelationshipQuery, nodes []*Task, init func(*Task), assign func(*Task, *Relationship)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Task)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(relationship.FieldSourceTaskID)
	}
	query.Where(predicate.Relationship(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(task.SourceRelationshipsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "source_task_id" returned %v for node %v`, fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (_q *TaskQuery) loadTargetRelationships(ctx context.Context, query *RelationshipQuery, nodes []*Task, init func(*Task), assign func(*Task, *Relationship)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Task)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(relationship.FieldTargetTaskID)
	}
	query.Where(predicate.Relationship(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(task.TargetRelationshipsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "target_task_id" returned %v for node %v`, fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (_q *TaskQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withCollection != nil {
			_spec.Node.AddColumnOnce(task.FieldCollectionID)
		}
