...
func TestMultiStore_LazyRegistration_ConcurrentSafety(t *testing.T) {
	...
	// Launch multiple goroutines that all trigger lazy resolution at
	// the same time.
	const goroutines = 10
	...
	for i := 0; i < goroutines; i++ {
		...
		go func(idx int) {
			defer wg.Done()
			ctx := context.Background()
			_, _, err := ms.ListTasks(ctx, store.ListTasksParams{CollectionID: &collID})
			errs[idx] = err
		}(i)
	}
	wg.Wait()
	...
	// cache should still work (no panics, no data races).
	if calls := resolverCalls.Load(); calls < 1 {
		t.Errorf("resolver calls = %d, want >= 1", calls)
	}
}
