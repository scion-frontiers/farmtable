...
//
// PROVENANCE AND RE-MEASUREMENT (#194 round 8). This file was written by the
// round-7 test-review leg, which was deleted before its commit reached a
// branch. It is restored here unchanged apart from this paragraph and gofmt.
//
// Restoring a file on somebody else's say-so that it is RED-capable is exactly
// the move this workstream keeps getting burned by, so it was re-measured on
// the r8 tree rather than taken on trust:
//
//	both error returns swallowed   -> RED 2 (both error-reporting tests), repo-wide
//	remove-side return swallowed   -> RED 2
//	add-side return swallowed      -> RED 1 (UpdateTask only; ClaimTask's swap
//	                                  reaches the remove arm first)
//
// Those are the leg's own reported numbers, reproduced. Before this file
// existed, all three mutations left `go test ./...` at exit 0.
//
// Note this file needed NO change for round 8's writeLabelSwap signature
// change (the new stageWritePolicy argument): it drives UpdateTask and
// ClaimTask, not the helper directly, which is why it survives a refactor of
// the thing it pins.
