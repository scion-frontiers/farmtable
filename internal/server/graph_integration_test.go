...
// strPtrG returns a pointer to s (avoids colliding with the helper in
// server_test.go when running with -count > 1).
...
// createTask creates a task via gRPC in the given collection with the
// specified blocking relationships and returns the proto.
func createTask(
	t *testing.T,
...
func TestGraphIntegration_GetCriticalPath_SimpleChain(t *testing.T) {
	...
	taskA := createTask(t, client, collID, "Task A", nil)
	taskB := createTask(t, client, collID, "Task B", []string{taskA.GetId()})
	taskC := createTask(t, client, collID, "Task C", []string{taskB.GetId()})
...
func TestGraphIntegration_GetReadyTasks_SimpleChain(t *testing.T) {
	...
	taskA := createTask(t, client, collID, "Task A", nil)
	createTask(t, client, collID, "Task B", []string{taskA.GetId()})
	createTask(t, client, collID, "Task C", []string{taskA.GetId()})
...
func TestGraphIntegration_GetBlockedTasks_SimpleChain(t *testing.T) {
	...
	// B and C should be blocked; A should not be blocked.
	blockedNames := make(map[string]bool)
	blockerMap := make(map[string]int) // name -> number of blockers
	for _, item := range resp.Items {
		name := item.Task.GetName()
		blockedNames[name] = true
		blockerMap[name] = len(item.BlockedBy)
	}
	...
	if !blockedNames["Task C"] {
		t.Error("expected Task C to be blocked (blocked by B)")
	}

	_ = taskC // referenced above via blockedNames
}
...
func TestGraphIntegration_GetBottlenecks_Diamond(t *testing.T) {
	...
	taskA := createTask(t, client, collID, "Task A", nil)
	taskB := createTask(t, client, collID, "Task B", []string{taskA.GetId()})
	taskC := createTask(t, client, collID, "Task C", []string{taskA.GetId()})
	createTask(t, client, collID, "Task D", []string{taskB.GetId(), taskC.GetId()})
...
func TestGraphIntegration_GetReadyTasks_IndependentTasks(t *testing.T) {
	...
	createTask(t, client, collID, "Independent A", nil)
	createTask(t, client, collID, "Independent B", nil)
	createTask(t, client, collID, "Independent C", nil)
...
func TestGraphIntegration_GetBlockedTasks_NoneBlocked(t *testing.T) {
	...
	createTask(t, client, collID, "Free A", nil)
	createTask(t, client, collID, "Free B", nil)
...
func TestGraphIntegration_GetBottlenecks_SimpleChain(t *testing.T) {
	...
	taskA := createTask(t, client, collID, "Task A", nil)
	taskB := createTask(t, client, collID, "Task B", []string{taskA.GetId()})
	createTask(t, client, collID, "Task C", []string{taskB.GetId()})
