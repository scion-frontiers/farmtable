 ...
 *      prove.
 *
 *   5. THE PROTOCOL GAP -- ADDED FOR #103, AND THE REASON THIS FILE CHANGED.
 *
 *      #195 and this branch each invented assertion-count pinning, separately,
 *      with incompatible conventions. Reconciling the two `npm test` lists puts
 *      suites using BOTH in one run. MEASURED, on the two heads:
 *
 *        A. `#assertions N` on stdout, from src/util/assertions.ts.
 *           Four suites on the XSS side.
 *        B. A PRIVATE TOTAL in the suite's own prose --
 *           `markdown sanitizer: 82 checks passed (131 assertions)` -- with the
 *           suite self-checking it against its own EXPECTED_ASSERTIONS. One
 *           suite, util/markdown, 5583 lines. `grep -c '#assertions'` on it: 0.
 *        C. SILENCE. #195's utils/task-ready.test.ts prints nothing at all on
 *           success and has no counter of any kind. It is not a bad suite; it
 *           simply predates both conventions.
 *
 *      The unmodified gate above treats B and C identically -- "emitted no
 *      receipt" -- so reconciling the lists without this change turns a working
 *      5583-line suite into a hard failure for using the other branch's
 *      convention. That is the #103 defect one level up: a report that was not
 *      UNDERSTOOD and a report that was never MADE become the same thing.
 *
 *      So the protocol is DECLARED PER SUITE in test-receipts.manifest.json,
 *      not sniffed. A suite absent from the manifest must use A. A suite that
 *      matches NEITHER its declared protocol nor A is a hard error naming the
 *      suite -- ** ZERO RECEIPTS IS AN ERROR, NOT A ZERO ** -- so a third
 *      convention cannot arrive quietly, the way the second one did.
 */
...
const manifestResult = loadManifest(webRoot);
if (manifestResult.error) {
  console.error(`FAIL: ${manifestResult.error}`);
  process.exit(1);
}
...
for (const s of sources) {
  ...
  const outcome = readAssertionTotal(s, stdout, manifest);
  if (outcome.error) {
    failed.push(`src/${s}.test.ts (${outcome.error})`);
  } else {
    perSuiteCounts.push([s, outcome.count]);
    totalAssertions += outcome.count;
  }
  console.log('');
}
...
// and why.
//
// ── #103: THE VALUE MOVED TO THE MANIFEST, AND IT IS CURRENTLY UNSET ─────────
//
// It was `380`, an exact equality over this branch's four suites. That number
// does not survive reconciliation: the union adds util/markdown (131 by its own
// count) and the one shared suite, utils/task-ready.test.ts, exists as two
// different blobs. The correct total is a property of a merge that has not
// happened and a run that has not been authorised, so it is `null` in
// test-receipts.manifest.json with the reasoning written beside it.
//
// It is NOT set by arithmetic. 380 + 131 is a plausible-looking number nobody
// has observed, and a pin asserted at a wrong value is worse than an absent
// one -- it reddens a correct tree, and whoever clears it will clear it by
// editing the pin, which teaches everyone that the pin is the thing that gives.
//
// AN UNSET PIN DOES NOT PASS QUIETLY. It prints, every run, at the end, saying
// what is unprotected. A gate that is off and silent is indistinguishable from
// a gate that is on and agreeing -- which is the whole of #103.
const EXPECTED_ASSERTIONS = manifest.aggregatePin;

if (EXPECTED_ASSERTIONS === null) {
  console.warn(
    '\n' +
      '  ┌──────────────────────────────────────────────────────────────────────────┐\n' +
      '  │ AGGREGATE ASSERTION PIN IS UNSET. This run did NOT check the suite total. │\n' +
      '  └──────────────────────────────────────────────────────────────────────────┘\n' +
      `  Observed total this run: ${totalAssertions}\n` +
      perSuiteCounts.map(([s, n]) => `    ${String(n).padStart(5)}  src/${s}.test.ts`).join('\n') +
      '\n\n' +
      '  UNPROTECTED while unset: an assertion deleted from a suite that still runs.\n' +
      '  Still protected: each suite\'s own internal pin, the per-suite zero/no-receipt\n' +
      '  gates above, and suite MEMBERSHIP (scripts/check-test-membership.mjs).\n\n' +
      '  To set it, paste this into test-receipts.manifest.json and commit it with the\n' +
      '  SHA it was measured at:\n\n' +
      `      "aggregateAssertionPin": ${totalAssertions},\n`,
  );
} else if (totalAssertions !== EXPECTED_ASSERTIONS) {
  ...
  console.error(
    `FAIL: the suite evaluated ${totalAssertions} assertions, expected exactly ` +
      ...
      (delta < 0
        ...
        : '  You added assertions. Update "aggregateAssertionPin" in\n' +
          '  web/test-receipts.manifest.json in the same commit, so the diff records it.\n') +
      '  Per-suite counts:\n' +
      perSuiteCounts.map(([s, n]) => `    ${String(n).padStart(5)}  src/${s}.test.ts`).join('\n'),
  );
  process.exit(1);
