...
type RelationshipQuery struct {
	...
	withSourceTask *TaskQuery
	withTargetTask *TaskQuery
...
// QuerySourceTask chains the current query on the "source_task" edge.
func (_q *RelationshipQuery) QuerySourceTask() *TaskQuery {
...
// QueryTargetTask chains the current query on the "target_task" edge.
func (_q *RelationshipQuery) QueryTargetTask() *TaskQuery {
	query := (&TaskClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
...
func (_q *RelationshipQuery) Clone() *RelationshipQuery {
	...
	return &RelationshipQuery{
		...
		withSourceTask: _q.withSourceTask.Clone(),
		withTargetTask: _q.withTargetTask.Clone(),
...
// WithSourceTask tells the query-builder to eager-load the nodes that are connected to
// the "source_task" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *RelationshipQuery) WithSourceTask(opts ...func(*TaskQuery)) *RelationshipQuery {
	query := (&TaskClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withSourceTask = query
	return _q
}

// WithTargetTask tells the query-builder to eager-load the nodes that are connected to
// the "target_task" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *RelationshipQuery) WithTargetTask(opts ...func(*TaskQuery)) *RelationshipQuery {
	query := (&TaskClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withTargetTask = query
	return _q
}
...
//		GroupBy(relationship.FieldSourceTaskID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Relationship.Query().
//		Select(relationship.FieldSourceTaskID).
//		Scan(ctx, &v)
...
func (_q *RelationshipQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Relationship, error) {
	var (
		...
		_spec       = _q.querySpec()
		loadedTypes = [2]bool{
			_q.withSourceTask != nil,
			_q.withTargetTask != nil,
		}
	)
	...
	if query := _q.withSourceTask; query != nil {
		if err := _q.loadSourceTask(ctx, query, nodes, nil,
			func(n *Relationship, e *Task) { n.Edges.SourceTask = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withTargetTask; query != nil {
		if err := _q.loadTargetTask(ctx, query, nodes, nil,
			func(n *Relationship, e *Task) { n.Edges.TargetTask = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *RelationshipQuery) loadSourceTask(ctx context.Context, query *TaskQuery, nodes []*Relationship, init func(*Relationship), assign func(*Relationship, *Task)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Relationship)
	for i := range nodes {
		fk := nodes[i].SourceTaskID
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "source_task_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *RelationshipQuery) loadTargetTask(ctx context.Context, query *TaskQuery, nodes []*Relationship, init func(*Relationship), assign func(*Relationship, *Task)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Relationship)
	for i := range nodes {
		fk := nodes[i].TargetTaskID
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "target_task_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *RelationshipQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withSourceTask != nil {
			_spec.Node.AddColumnOnce(relationship.FieldSourceTaskID)
		}
		if _q.withTargetTask != nil {
			_spec.Node.AddColumnOnce(relationship.FieldTargetTaskID)
		}
	}
