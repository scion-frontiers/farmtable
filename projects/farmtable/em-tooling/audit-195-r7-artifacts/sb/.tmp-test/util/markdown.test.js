...
function spoofingAttributes() {
    ...
    check('DOM-clobbering id/name attributes stripped', () => {
        const out = renderMarkdown('<a name="body">a</a><a id="body">b</a><p id="children">c</p>' +
...
function renderMarkdownArityViolation(src) {
    ...
    if (params.length !== 1) {
        return (`renderMarkdown declares ${params.length} parameters: (${decls[0][1].trim()}) — a second ` +
...
function splitTopLevelDefault(param) {
    let depth = 0;
    for (let i = 0; i < param.length; i += 1) {
        ...
        if (c === '(' || c === '[' || c === '{' || c === '<')
            ...
            depth = Math.max(0, depth - 1);
        else if (c === '=' && depth === 0 && param[i + 1] !== '=' && param[i - 1] !== '=')
            return true;
    }
    return false;
}
...
function sinkBinding() {
    ...
    check('fixture: a string literal naming the sink identifiers is not a violation', () => {
        const withMessage = SOUND_SINK_FILE.replace('export class C extends LitElement {', "export const MSG = 'always call renderMarkdown before unsafeHTML';\n" +
            'export class C extends LitElement {');
...
function dependencyPolicy() {
    ...
    check('sunset clause: #204 tooling absent, tokenizer subset still earns its place', () => {
        const eslintDeps = Object.keys(deps).filter((d) => d === 'eslint' || d.startsWith('@typescript-eslint/') || d === 'typescript-eslint');
