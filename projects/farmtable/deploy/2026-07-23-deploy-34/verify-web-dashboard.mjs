// Playwright verification script for farmtable web dashboard auth (deploy-34, Check d)
// Tests: login flow, session cookie, logout, and confirms no ?token= URL param requirement.
...
const FT_TOKEN = process.env.FT_TOKEN || 'ft_a117d8f3ab79f3d8a78248cb47136a8bbf5dbd3826c40108b08c4b850da8f699';
...
async function run() {
  ...
  try {
    ...
    console.log('\n=== Check d1: Load web dashboard without ?token= param ===');
    {
      ...
      try {
        ...
        if (status === 200) {
          ...
          const hasLoginDialog = bodyText.includes('login') || bodyText.includes('Login') ||
                                  bodyText.includes('token') || bodyText.includes('Sign in');
          await page.screenshot({ path: `${EVIDENCE_DIR}/d1-dashboard-no-token.png` });
          record('d1', 'Load dashboard without ?token= param', true,
            `Dashboard loaded (HTTP ${status}). URL: ${url}. ${hasLoginDialog ? 'Login dialog detected' : 'App UI rendered'}.`);
        } else {
    ...
    try {
      ...
      // d5: Dashboard loads with session
      console.log('\n=== Check d5: Dashboard loads data with session cookie ===');
      await page.goto(SERVICE_URL + '/', { waitUntil: 'networkidle', timeout: 30000 });
      ...
      if (hasContent) {
        record('d5', 'Web dashboard loads data with session cookie', true,
          `Dashboard rendered (body length=${pageContent.length}). Session-to-bearer middleware working.`);
      } else {
        record('d5', 'Web dashboard loads data with session cookie', false,
          `Dashboard empty. body length=${pageContent.length}`);
      }
