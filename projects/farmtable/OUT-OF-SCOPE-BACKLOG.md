...
### NUMBERING COLLISION, NOT RESOLVED BY DESIGN (S20-S23 EXIST TWICE)
coordinator and em-ci appended to this file within the same minutes and both allocated
S20-S23. Per the standing rule that produced the duplicate C7 rows, NOTHING IS
RENUMBERED — a citation already made against either row must keep resolving. Cite these
by TITLE, never by number:
  S20 "THE FIRST SWEEP SAW 1% ..." (em-ci)   vs  S20 "THE 63 WERE PRESERVED ..." (coord)
  S21 "A REPORTED SHA IS A HANDOFF ..." (em-ci) vs S21 "A PRESERVATION IS WORTH ..." (coord)
  S22 "THE DELIVERABLES OF THIS TRACK ..." (em-ci) vs S22 "THE CORRECTED ENUMERATOR ..." (coord)
  S23 "DRIFT MEASURED ..." (em-ci) vs S23 "THE LEG CONTRADICTED THE EM ..." (coord)
Both sets survived: read-modify-write at write time, diffed current-minus-mine, 0 rows
deleted on both appends. THE COLLISION IS THE COST OF THE ONLY SAFE CONCURRENT WRITE
DISCIPLINE WE HAVE, and it is cheaper than a lost row. A monotonic allocator would fix
it and is out of scope.

### A11-a. `git add` SILENTLY SKIPS NESTED `.git` DIRS AND EXITS 0 — HAZARD, NOT YET A LOSS
Addendum to A11, filed by dev-194-pricing, NOT FOLLOWED. Disclosed by `farmtable-em-ci`
against itself while snapshotting the scratchpad: git IGNORED 56 paths inside the nested
`.git` directories of two vendored reference clones, mentioned it in passing, and EXITED 0.
Cost today: nothing — they were reference clones, both shallow depth-1, heads == origin/main,
clean, SINGLE-COPY COUNT ZERO (measured by em-ci as enumerated-minus-in-tree, not read off
the warnings: exactly 56, all inside `reference/beads/.git` and `reference/watcher/.git`,
zero outside a nested `.git`).
~~THE FORWARD HAZARD IS THAT THE SAME SILENCE IS TOTAL AND LOOKS IDENTICAL: if any leg ever
clones into the scratchpad, its entire repository is invisible to the snapshot mechanism
WHILE THE SNAPSHOT REPORTS SUCCESS.~~
**STRUCK 16:05Z — WRONG, AND WRONG IN THE DIRECTION THAT UNDERSTATED IT.** Corrected by
`farmtable-em-ci` BY REPRODUCTION, not by argument: it built a nested repo holding a commit
existing nowhere else and ran the pipeline. Exit 0. Positive arm `only-copy.txt` **CAPTURED**;
negative arm `history-only.txt`, committed on a branch not checked out, **DROPPED**.
**SO THE WORKING TREE IS CAPTURED IN FULL AND WHAT DIES SILENTLY IS EVERYTHING NOT CURRENTLY
CHECKED OUT** — history, other branches, stashes, a file deleted in HEAD but alive earlier,
and the entire account of who did what. **THE SNAPSHOT PRESERVES A LEG'S FILES AND DESTROYS
ITS ACCOUNT OF THEM, AND THE DIRECTORY LOOKS COMPLETE TO WHOEVER OPENS IT.** That is nastier
than the total loss I claimed, because total loss at least announces itself. Filed by em-ci
as **S25** (cite by title; the numbers collide). Day's dominant defect class — the signal
exists and carries no information — and note that I reached for the more dramatic failure
when the quieter one was the real one.
Also corrects A11 as originally filed: dev-194-pricing wrote "no sweep carries them",
which UNDER-CALLED it. The measured fact is worse — the scratchpad is a git repo with
ZERO remotes and every farmtable doc UNTRACKED, 995 dirty entries. They were not merely
unswept; they were never committed anywhere. A11 was stopped at the mechanism visible
from one side; the number came from someone checking instead of agreeing.

### S24 (em-ci). A RUN ID IS EVIDENCE ONLY WHILE THE COMMIT IT RAN ON IS RETRIEVABLE
Raised by farmtable-ci-workflow. It reported CLEAN and on a branch, then volunteered the
opposite of the failure it had been warned about: its four PRE-REBASE commits (bbc9a59,
2e5fcaa, 5893676, 91ad031) had become reflog-only in its tree after a `-B` repoint, and
it asked whether origin still held them, "because the evidence is attached to them and
to nothing else."
It did. But the check answered a bigger question than it asked. TEN OF THE TWELVE
COMMITS CITED BY RUN ID IN status-ci.md WERE REACHABLE FROM EXACTLY ONE MUTABLE BRANCH
ON ORIGIN — every ci.yml canary, both pre-rebase and shipping-tree, plus the floor,
additions and substitution arms. One force-push or branch delete and the commit a cited
RED was produced from is gone, WHILE THE RUN ID GOES ON RESOLVING IN THE GITHUB UI AND
POINTING AT NOTHING YOU CAN CHECK OUT. The citation does not break; it hollows out.
The two exceptions were the main fast-forwards (10 and 6 containing refs) — i.e. the
evidence was durable exactly where it was least at risk and single-pointered everywhere
it mattered.
FIXED, NOT JUST FILED: 12 immutable refs pushed as
`refs/salvage/ci-evidence/run-<id>-<name>`. Verified after prune-fetch: each cited
commit now has a second, non-branch pointer. Recorded in status-ci.md with the fetch
refspec.
GENERAL FORM: WHEN YOU CITE A CI RUN AS EVIDENCE, PIN THE COMMIT OUTSIDE refs/heads IN
THE SAME ACT. A branch is a bookmark, not an archive, and the difference is invisible
until someone tidies up.
CREDIT: ci-workflow, for reporting a risk in its own work that nobody had asked about
and that reflected badly on nothing but the process. That is the second leg today whose
unprompted disclosure beat its EM's instrument.

### S25 (em-ci). THE 56 DROPPED PATHS: ZERO SINGLE-COPY, AND THE HAZARD IS NARROWER AND WORSE THAN STATED
Assigned by coordinator ("disclosure is not resolution"). Measured, not asserted.
WHAT THEY WERE: exactly 56, derived as enumerated-minus-in-tree, not read off the
console warnings. 28 under `reference/beads/.git/`, 28 under `reference/watcher/.git/`.
ZERO dropped paths outside a nested `.git`.
ARE ANY SINGLE-COPY? NO, and here is why rather than merely that: both are shallow
depth-1 clones of public upstreams. For each, `refs/heads/main` == `refs/remotes/origin/main`
== HEAD, one shallow boundary, one reflog tip, working tree clean. No local commit
exists in either. Nothing was at risk. THE INTEGER IS 0.
BOTH ARMS, IN A PURPOSE-BUILT REPRODUCTION rather than an argument: built a nested repo
holding a commit that exists nowhere else, ran THE SAME pipeline, `update-index` exit
code 0.
  positive arm: `legclone/only-copy.txt` (working tree)     CAPTURED
  negative arm: `legclone/history-only.txt` (committed on a
                branch that is not checked out)             DROPPED
THIS CORRECTS dev-194-pricing's forward hazard, in the direction that makes it sharper.
It is NOT "if a leg clones into the scratchpad its entire repository is invisible." The
WORKING TREE IS CAPTURED IN FULL. What is silently lost is everything that is not
currently checked out: history, other branches, stashes, a file deleted in the current
commit but alive in an earlier one, and the identity of who committed what and when.
So the snapshot preserves a leg's FILES and destroys its ACCOUNT OF THEM, and a reader
of the snapshot cannot tell the difference, because the directory looks complete.
A COMMAND THAT DROPS DATA AND EXITS 0 IS THE FOURTH SIGN OF THE 148 CLASS. The other
three concerned a local proxy standing in for the network; this one is a tool reporting
success over a documented, deliberate, silent exclusion (`.git` dirs are excluded BY
DESIGN in git; the design is right and the silence is the defect).

### S26 (em-ci). WEB tsc IS NARROW BY POPULATION, NOT BY INSTRUMENT — RELAYED, NOT MINE
From em-task-state: the web `tsconfig` `include` is `src` only, so 22 of the 30 files in
the pending merge ARE NOT TYPECHECKED AT ALL. dev-p2-merge proved the instrument is LIT
with a positive control (a real TS2305 inside `src`), so THE DEFECT IS THE POPULATION,
NOT THE COMPILER. Same shape as the web membership gate: `web/tsconfig.test.json`
`include` is `src/**/*.test.ts(x)|spec.ts(x)`, which is why a `.test.mts` enters the
membership population and is never emitted.
TWO GATES, ONE ROOT: the web toolchain's idea of "the project" is `src`, and everything
outside `src` is ungated while looking gated. Filed with the C14 family (floor-not-a-
manifest). NOT FIXED — design change, scope frozen.

**C21 — A CARVE-OUT SCOPED BY CONFLICT STATUS IS SCOPED BY THE WRONG PROPERTY.** The
safe-url carve-out named the two conflicted files. **Five files carry safe-url policy.**
Provenance at `main=2982ffd` / `branch=e64138c` / `base=aa08f1a`:

```
base=no main=yes branch=yes   web/src/util/safe-url.ts               [carved]
base=no main=yes branch=yes   web/src/util/safe-url.test.ts          [carved]
base=no main=no  branch=yes   web/test/safe-url.contract.test.ts        <-- missed
base=no main=no  branch=yes   web/test/ft-inspector-code.safe-url.test.ts <-- missed
base=no main=no  branch=yes   web/test/ft-inspector-meta.safe-url.test.ts <-- missed
```

The three missed files encode the branch's safe-url semantics as surely as the
implementation does. They escaped the carve-out **only because both sides did not happen
to write the same path** — being pure branch-side additions they are not conflicted, so
they merge unconditionally and then fail against main's implementation (30 of 422 tests).
**Conflict status is textual path overlap; a carve-out needs semantic coupling.** The two
diverge exactly when one side originates a file the other never touched, which is the
common case for new tests. Generalisation: **scope a carve-out by what the files mean, then
check conflict status — never the reverse.**

**C21a — acceptance (a) was mis-specified, and the conflict it produced looked like a merge
problem.** "Leave the two safe-url files at main's version" reads as a neutral deferral and
is in fact a pick-a-side — the precise act the ruling forbade, made unrecoverable by the
absent stage 1 (C19). The report framed this as "(a) and (c) cannot both hold," which
invites resolving it by weakening (c). The correct reading is that (a) is wrong. **Recorded
because the failure presented as an instruction conflict rather than as a defective
instruction, and those need different fixes.**

**C21b — the tempting close is the dangerous one.** Landing the branch's `safe-url.ts`
turns the suite 422/422 green and silently adopts the branch's policy. Comfort direction
again (R5, C18, C20a): **the resolution that makes the alarm stop is the one that decides
the security question without anyone ruling on it.** Recommended instead: extend the
carve-out to all five, defer together, accept a visible temporary floor move 30 -> 27.

**C21c — credit where the method held.** dev-p2-merge did NOT infer safety from the failure
count. It checked every renders-no-href assertion individually (`javascript:`,
`JaVaScRiPt:`, whitespace-padded, `data:`, `vbscript:`, `file:`) and established that the
residual string matches are the raw URL inert inside a `title` attribute, never in an
`href`. **Thirty red tests with zero security regression** — the number alone would have
justified panic, and naming the assertions is what prevented it.

**R4a UPGRADED FROM CONJECTURE TO DEMONSTRATED — em-ci, canary 5d9df1f, run 30467223768.**
When I filed R4a I could describe substitution but had no instance. em-ci produced one at
main `2982ffd`: delete `web/src/utils/task-ready.test.ts`, add a compilable replacement in
the same commit. Result **SUCCESS**, `enumerated=6 executed=6 missing=0 (floor 6)`. The
gate reported full health across a swapped suite. Stronger corollary from the same numbers:
**at floor 6 over a 27-file population, deleting 21 web test files passes green.** The
theoretical gap and the live exposure turned out to be the same gap.

**C22 — A SCALAR FLOOR CANNOT BE REPAIRED BY RAISING IT, AND RAISING IT MAKES THINGS WORSE.**
The reflex fix is floor 6 -> 27. That is the identical defect at a larger number, and it is
more dangerous because 27 *reads* as coverage while 6 visibly does not. **A floor is a
scalar; a manifest is a set.** Only a committed name set discriminates. The working
instrument is em-task-state's pre-registration (C14): 30 names predicted from the two
parents and the base **before** the merged tree was enumerated, actual 30, missing=0
unexpected=0, with a discrimination arm that dropped one name and returned missing=1
*naming the file*. Prediction-before-enumeration plus a lit discrimination arm — that is the
gate; the floor is bookkeeping that follows the population afterward.

**C22a — MY OWN ERROR, AND IT WAS THE EXPENSIVE KIND: I INVENTED A COST.** I told
em-hardening that extending the carve-out to five files would cost "a temporary floor move
30 -> 27," and offered it as the counterweight to weigh against adopting a policy by
accident. **There was no such cost** — the floor is 6, and 6 <= 27 is green either way.
em-ci corrected it and I withdrew it within the minute.

The lesson is not the wrong number. Every other error in this review was a *measurement*
that could be re-run and disconfirmed — the unrebased tip (C19), the subject-based dedupe
(C20a), the zsh zero. **This one was a cost asserted from plausibility and never measured
at all**, which is why nothing in my own method caught it: there was no instrument to
control, because I had run none. I checked the merge report's `floor 30` and never asked
what the floor was on main. **A fabricated trade-off is more corrosive than a wrong
measurement, because it survives review — it looks like judgement rather than data, and
judgement is not something reviewers re-run.** Rule for myself and worth generalising:
**if a recommendation names a cost, that cost is a measurement and must carry a SHA like any
other.** Had em-hardening weighed it, an illusory cost would have sat on the side of
adopting a security policy silently.

### S27 (em-ci). THE OVERLAY BOUNDARY, DERIVED BY A LEG, AND THE EM IS IN THE FAILING CLASS TOO
Asked both held legs the coordinator's two questions with a same-invocation control.
Both answered clean, and both gave more than was asked.
ci-22-setup derived the PORTABLE FORM, which is the useful artefact because it needs no
measurement: **/tmp and /var/tmp ARE the container overlay and are invisible to any EM
sweep; /workspace and /scion-volumes are /dev/root ext4 and are not.** It had made four
scratch clones, all under /tmp, all deleted, and said so plainly: "I got there by habit,
not by knowing this." A HABIT IS NOT A SAFETY PROPERTY.
ci-workflow cross-checked with /proc/mounts as a SECOND AXIS WITH A DIFFERENT FAILURE
MODE rather than re-reading one number, and then volunteered 25 repos under its own
overlay /tmp: 14 disposable verification clones, 11 harness-scratch repos whose root is
not farmtable's b0f610d4, and ONE genuinely unique farmtable commit, 8d7b305 (/tmp/w2h/
absent), existing nowhere else on earth. Its own recommendation was NOT to preserve it -
correct on the merits, since its successor 55afbab is a real branch and no cited run
hangs on it. OVERRIDDEN ON DIFFERENT GROUNDS: it is the only confirmed container-only
unique commit in this half of the fleet, so its value is as a SPECIMEN OF THE CLASS, not
as evidence about web/dist. Being preserved via its host-backed clone.
Its line is kept verbatim because it is the general rule: "DECIDING THEY WERE
UNINTERESTING IS EXACTLY THE JUDGEMENT THAT LOSES THE ONE THAT WAS NOT."
THE EM IS NOT EXEMPT. Ran the same test on myself: **my own /tmp is device 61, overlay,
holding six repos.** Coordinator's is 42. The numbers differ per container, which is
precisely why the control must be in the same invocation and why no value can be
hard-coded. Every one of my six was measured against origin: off-origin = 0, except the
two commits in the purpose-built S25 reproduction, which are supposed to be off-origin
and are the negative arm working.
GAP CLOSED IN PASSING: ci-22-setup found dbc1ced (docs/floor-predicate-r2) held by
exactly one mutable scratch ref in canonical, on no remote - created AFTER my sweep. The
staleness rule caught me a third time. Pushed to
refs/salvage/farmtable-ci-manifest/docs/floor-predicate-r2, verified ON-ORIGIN with a
lit negative arm. It offered to bundle it into the shared volume and asked first; the
bundle was unnecessary because the object was already in my tree, but ASKING BEFORE
WRITING TO SHARED STORAGE WAS RIGHT.

**C23 — THE SAFE-URL RULING IS NOT IMPLEMENTABLE AS A RECOMBINATION OF THE TWO SIDES.**
Ruling: *allow by default with a per-deployment switch*, characterised as main's default plus
the branch's mechanism, inverted. Measured at `e64138c`, blob `d85bb5b`:
...
`import.meta.env.DEV` is **statically replaced by Vite at build time**. It is a build-time
constant, not a deployment switch — flipping it requires a rebuild. Every other gate in the
client is the same class (`ft-app.ts:319`, `grpc-client.ts:430`, both `import.meta.env.DEV &&
VITE_ENABLE_LOCAL_TOKEN`; `VITE_`-prefixed vars are also baked in). **Census of runtime-config
sources in `web/src`: zero.** No served config endpoint, no meta tag, no window global.

So the branch does not own "the mechanism" and main does not own "the default in a switchable
form." **Neither side implements the ruling, and the codebase has no precedent for the class of
thing the ruling names.** Somebody must design where runtime configuration comes from; that
work sits outside the five carved files and outside the merge. The "one to two cycles" estimate
predates this and should be re-taken. Second, smaller gap: the branch flag even when ON admits
only `LOCAL_HOSTNAMES`, so it is loopback-only, not http-generally — narrower than "allow" on a
second axis.

**C23a — TWO FLAGS THAT LOOK IDENTICAL HAVE OPPOSITE REQUIREMENTS. DO NOT GENERALISE.**
`grpc-client.ts:424` states the property on purpose: DEV-gating makes the local-token path
*"never readable from script in a production bundle."* That is a **build-time** guarantee, and
it is strictly stronger than anything a runtime switch can offer. safe-url needs to become
runtime-configurable; the token fallback must NOT. A sweep that "makes the flags configurable"
would silently downgrade a deliberate security property. Filed because the two flags are
written in the same idiom, sit three files apart, and the correct treatment is opposite.

**C24 — "UNION THE devDeps" IS UNDERSPECIFIED, AND IT IS MY DEFECT.** My §5 merge
recommendation said take the branch's `scripts` block and union the devDeps. Measured:

```
main   2982ffd   "jsdom": "^29.1.1"                      (no vitest)
branch e64138c   "jsdom": "^26.1.0",  "vitest": "^3.2.7"
```

`web/package.json` is conflicted, and **union does not define an answer when one dependency
carries two versions.** Main is three majors ahead; a mechanical resolution toward the branch
downgrades jsdom 29 -> 26 while looking like it merely took the recommended side. Correct
resolution: main's `^29.1.1` plus vitest, and *confirm* the 22 component tests pass on 29
rather than assume it. Generalisation: **a merge instruction phrased as a set operation must
say what happens when the same key appears twice.** Mine did not.

**C25 — BRIEFING §2c RE-DERIVED AND CLEARED, ONE WORD WITHDRAWN.** The coordinator withdrew
the harness column wholesale (author's own retraction; the "branch has no DOM harness" claim
was false, the config split was deliberate, denominator 81 vs a true 91 — every component
running toward the requesting track). I had relied on a harness claim, so I re-derived from my
own measurements: main has **no vitest, no `vitest.config.ts`, zero `web/test` files**; the
branch has all three plus `environment: 'jsdom'`. **The conclusion stands on my evidence, not
on the withdrawn report.** But main *does* declare `jsdom ^29.1.1` and `@types/jsdom`, so the
quoted "no browser/DOM test harness **at all**" is too strong at current main: the dependency
is present, no runner is wired to it. Withdrawing "at all"; substance unchanged.

The transferable part is the check, not the outcome. **When a source you cited is withdrawn,
re-derive rather than reason about whether the withdrawal reached your particular claim.** It
cost one command here and it found a real overclaim in my own text — which the coordinator's
withdrawal notice, being about someone else's column, would never have surfaced.

### S28 (em-ci). THE TYPECHECK POPULATION, MEASURED AT TWO TREES, AND THE RED ARM CAME BACK GREEN
Ordered by coordinator: where the compile canary was placed, the two integers with the
enumerator published, and a red arm injected OUTSIDE the include ("if it comes back
green, report it and do not fix it").
WHERE MY CANARIES WERE: **INSIDE `src`, both of them** — `canary-added.test.mts` under
`web/src/util/`, `canary-substitution.test.ts` under `web/src/utils/`. So neither ever
probed outside the include, and I had no standing to relay the typecheck claim as though
I had checked it.
ENUMERATOR (published, per the rule): `git ls-tree -r` AT THE COMMIT, `web/**`,
`*.ts|tsx|mts|cts`, minus `web/dist`. VISITED = `tsc --noEmit --listFiles -p
web/tsconfig.json`, node_modules stripped. First attempt measured the CHECKED-OUT BRANCH
instead of the commit and was thrown away — the rule caught its own author.
  main 2982ffd         EXIST 59  VISITED 58  UNVISITED 1   (vite.config.ts)
  merged tree 42a71d8  EXIST 90  VISITED 62  UNVISITED 28  (26 web/test + 2 configs)
**THE GAP AT MAIN IS ONE FILE AND THAT IS THE TRAP.** Main has no `web/test` directory,
so the include pattern looks adequate precisely where it is never exercised. THE
BLINDNESS IS LATENT AT MAIN AND BECOMES LOAD-BEARING THE INSTANT THE MERGE LANDS. A
pattern is not validated by a tree that has nothing outside it.
RED ARM: `canary/typecheck-outside-include` = b11f49c on 2982ffd, real type error in
`web/vite.config.ts`. **RUN 30469490377: SUCCESS.** Pre-registered as green before the
push. Local both arms at 2982ffd: outside include -> `tsc` exit 0; identical error inside
`src` -> exit 2. NOT FIXED, as ordered.
Design note kept because it is the reusable part: the error was deliberately NOT planted
in a `web/test/*.test.ts`, because that file also enters the membership gate's
tree-derived population, and a red there would not discriminate THE TYPECHECK'S
BLINDNESS FROM THE MEMBERSHIP GATE'S SIGHT. A canary that two gates can claim is not a
canary.

### S29 (em-ci). A TREE THAT CANNOT BUILD STILL YIELDS CLEAN FILE COUNTS
Found while measuring S28, relayed to em-task-state as live. **42a71d8 is the RAW
`merge-tree --write-tree` output and carries CONFLICT MARKERS in seven blobs** — the same
seven in the settled conflict set. Measured consequence: `web/package-lock.json` is
INVALID JSON (parse fails at line 20), so `npm ci` dies EUSAGE, so `make web` cannot run,
so nothing in that tree builds or typechecks.
THE PRE-REGISTERED NAME SET SURVIVES THIS INTACT — enumerating paths does not require the
tree to build, and 30/30/missing 0/unexpected 0 with a fired discrimination arm all
stand. WHAT DOES NOT SURVIVE IS READING THAT EMPTY DIFF AS "THE MERGE IS GREEN."
A NAME-SET RESULT ON AN UNBUILDABLE TREE AND A NAME-SET RESULT ON A BUILDABLE ONE PRINT
THE SAME INTEGERS. Publish buildability in the same sentence as the name set or the next
reader will pool them — the same discipline as SHA-next-to-the-integer and
clock-next-to-the-zero, one level up.
This is the fifth instance of the day's shape on one page: I measured a tree that cannot
build and got clean file counts out of it.

## COORDINATOR ROWS, 2026-07-29 17:00Z. CITE BY TITLE, NOT NUMBER.
Numbering has collided twice between me and em-ci. Nothing is renumbered, per the C7
precedent. These carry a CO- prefix so they can never collide again. That prefix is itself a
row: two parties wrote to one shared resource with no agreement on naming, which is the same
defect as the preserve-namespace one, one floor up in the record instead of in the refs.

CO-A. THE STORE IS THE VARIABLE, NOT THE OBJECT.
Two findings landed within ten minutes from opposite directions and they are one finding. One
track found a store with NO REMOTE. Another found stores NO SWEEP CAN READ, work on ordinary
HEAD and branch refs inside repos on a leg's container-local overlay. Both reduce to: THE
ARTEFACT SITS SOMEWHERE THE DURABILITY MECHANISM DOES NOT REACH. Object reachability was never
the variable; STORE reachability is. Every row of my 222-tip composition breakdown was sorting
within the visible set, which is why the breakdown looked like coverage.

CO-B. VERSION CONTROL PRESENT, CORRECTLY OPERATED, WORTH NOTHING.
Measured by me with a lit negative arm: the shared scratchpad IS a git repo, THIRTY-THREE
COMMITS, THIRTEEN TRACKED FILES, ZERO REMOTES. Positive control on the code repo returned 1
remote, so the instrument works. I was told every project doc was untracked; that is false,
and THE CORRECTION MAKES IT WORSE. Thirteen tracked files out of 11,936. THOSE THIRTEEN ARE
MORE DANGEROUS THAN THE 11,923, BECAUSE THEY ARE THE ONLY ONES THAT LOOK COVERED. Same shape
as preserving into a namespace nobody fetches, moved up one level: committed into a repo
nobody clones. A MECHANISM THAT IS PRESENT AND CORRECTLY OPERATED IS NOT EVIDENCE THAT IT IS
CONNECTED TO ANYTHING.

CO-C. ONLY THE LEG CAN ANSWER, AND SWEEPING HARDER IS THE MOVE THAT CANNOT WORK.
The visibility direction is ASYMMETRIC: a container reads the shared volumes, the host side
cannot read a container's overlay. So "is anything of mine container-only?" is answerable ONLY
by the leg. Overlay device ids measured across six legs were all different, so there is no
value to grep for; the only portable test is the clone's device id compared IN THE SAME
INVOCATION against a known host-backed path. My own coordinator scratch directory sits on a
container-local device. EVERY MESSAGE AND BRIEF FILE I WROTE TODAY IS SINGLE-COPY, and I am
the one node no EM sweeps. I did not find this; the test two EMs forced on me found it.

CO-D. A COMPLETED SIGNAL IS AN ASSERTION ABOUT WORK, NEVER ABOUT ITS LOCATION.
A merge deliverable and its project log were single-homed on one container's disk and the leg
reported COMPLETED THIRTY SECONDS LATER. COMPLETED is the signal I use to authorise deletion.
The window between a deliverable existing in exactly one place and its author announcing it
finished is about half a minute, with a delete command waiting on the far side. The only thing
that saved it was the resolve-and-fetch-before-deletion order, WHICH WAS WRITTEN FOR A
DIFFERENT REASON THAN THE ONE THAT CAUGHT THIS.

CO-E. TWO INSTRUMENTS CAN SHARE A BLIND SPOT AND WE CAN NOW NAME WHY THIS PAIR DOES.
fsck TREATS REFLOGS AS ROOTS, so it structurally cannot report reflog-only work; for-each-ref
sees no ref. Run together they corroborate each other and are blind to the same class. On one
tree the reflog walk gave 4 tips where the correct walk finds 1062, and a published row rested
on the 4. Stronger than earlier instances of this class because the shared cause is NAMED
rather than merely observed. Also: 15 of 17 recovered commits were STASH-type, and nobody here
has been counting stashes as work.

CO-F. THE CARVE-OUT BOUNDARY WAS DRAWN BY CONFLICT STATUS, AND CONFLICT STATUS IS TEXTUAL PATH
OVERLAP, NOT SEMANTIC COUPLING.
Five files carry safe-url policy; the carve-out covered two. The other three are pure
branch-side additions, absent from base and from main, therefore unconflicted, therefore they
MERGED UNCONDITIONALLY and nobody looked. They slipped through solely because the two sides
did not happen to write the same path. A CLEAN MERGE IS EXACTLY THE CONDITION UNDER WHICH THIS
HOLE IS INVISIBLE. Applies to every carve-out this project has scoped. The EM's generalisation
is the one to keep: THE UNIT OF A DECISION IS WHATEVER THE TWO SIDES DISAGREE ABOUT, NOT
WHATEVER THE CONFLICT SET HAPPENS TO LIST.

CO-G. A STARTING TREE IS NOT A NEUTRAL PARTY.
A base was chosen and described to me as neutral, "the merge base, neither main nor the branch
tip." It was neither the merge base nor neutral: an ancestor of nothing current, and FOUR OF
ITS FIVE SAFE-URL BLOBS WERE BYTE-IDENTICAL TO THE BRANCH TIP. Choosing it was a pick of one
side BY BLOB, made in good faith by someone who believed it was neutral, because they were
looking at lineage while the policy rode along unexamined. The replacement revision has the
same property and I have made explicit re-derivation of all five files binding rather than
letting the base carry the policy again.

CO-H. THE OWNER'S ANSWER WAS A THIRD POSITION AGAIN, AND I BUILT A BINARY OPTION SET AGAIN.
He ruled ALLOW BY DEFAULT WITH A PER-DEPLOYMENT SWITCH. Main has the default and no switch;
the branch has the switch defaulted off. The decision is one side's default with the other's
mechanism, inverted, and NEITHER IMPLEMENTS IT. I asked an EM to pick main or the branch. I
had filed the rule against constructing an option set from the two things in front of me
EARLIER THE SAME DAY. Also caught: an independent reader inferred "allow by default means main
wins" by dropping the second half of the sentence, which is the available inference and is
wrong. THE SECOND CLAUSE OF A TWO-CLAUSE DECISION IS THE ONE THAT DOES NOT TRAVEL.

CO-I. FOUR MECHANISMS, ONE SHAPE, ALL GREEN.
The web membership gate derives its expected set FROM THE TREE, so a deleted file leaves the
expected set with the tree. The typecheck derives its population from an INCLUDE PATTERN, so a
file outside it is not merely unchecked but UNCOUNTED. The floor is a SCALAR standing in for a
set, and at floor 6 over 27 files, deleting 21 web test files passes green, demonstrated by
canary rather than argued. The carve-out was drawn by TEXTUAL CONFLICT standing in for
semantic coupling. FOUR DIFFERENT MECHANISMS, ONE SHAPE: A PROXY THAT WAS CORRECT ON THE AXIS
SOMEBODY CHECKED AND SILENT ON THE AXIS THAT MATTERED. Nobody was careless in any of the four.

CO-J. THE SNAPSHOT PRESERVES A LEG'S FILES AND DESTROYS ITS ACCOUNT OF THEM.
The working tree is captured in full; what dies silently is everything not currently checked
out, history, other branches, stashes, files deleted in HEAD but alive earlier, and the whole
record of who did what. The directory looks complete to whoever opens it. WORSE THAN TOTAL
LOSS, WHICH AT LEAST ANNOUNCES ITSELF. Established by reproduction with both arms, not by
reading console warnings, and it corrects the looser version of the hazard.

CO-K. AN ERROR WHOSE EVERY COMPONENT RAN ONE WAY, TOWARD THE REQUESTING TRACK.
A measurement supporting one side of a merge decision was withdrawn by its own author: the
claim that one side had no DOM harness was false, a deliberate documented configuration split
was reported as failures, and the denominator was 81 against a true 91. Every component
flattered the requester's own track. Not deference. UNDER-SEARCHING THAT LANDS ON THE
INTERESTED PARTY'S PREFERRED ANSWER IS INDISTINGUISHABLE FROM DEFERENCE FROM OUTSIDE, and that
is a fact about the evidence rather than an accusation about anyone. Treated as WITHDRAWN, not
corrected. A decision was fifteen minutes from resting on it.

CO-L. I APPLIED ONE OF MY OWN RULES OUTSIDE THE CASE IT WAS DERIVED FROM AND NEARLY LOST A
SEND. I had ruled that flag-help output is not evidence of non-delivery. That was derived from
group-channel sends where a delivery count is printed. A message to the owner returned flag
help WITH NO DELIVERY LINE, twice, and my rule said ignore it. Compounded: I read the TAIL of
the output, which was help text, instead of the HEAD, which was the actual error naming a
missing companion flag. A RULE THAT SAYS IGNORE THIS SIGNAL IS THE MOST DANGEROUS KIND TO
CARRY OUT OF ITS DERIVATION, and truncating a diagnostic from the wrong end hides the one line
that was written to be read first.

CO-M. THE ONE POSITIVE ROW. FOUR PARTIES CONVERGED ON THE SAME ANSWER, INDEPENDENTLY, INSIDE
FIFTEEN MINUTES, EACH HOLDING A DIFFERENT REASON, AND THE REASONS COMPOSED RATHER THAN
REPEATED: the option set was never two; the carve-out was scoped by the wrong predicate; the
supporting evidence had been retracted; the cost of waiting was zero. THREE OF THE FOUR
INCLUDED A CORRECTION OF THE SPEAKER'S OWN PRIOR STATEMENT. Two of the four retracted the same
figure within two minutes of each other without knowing the other had. Record what produced
this before assuming it is the normal yield of asking four parties the same question.

CO-N. THE MECHANISM WAS NAMED CORRECTLY AND SCOPED AT THE WRONG BINDING TIME.
The owner decided "allow by default WITH A PER-DEPLOYMENT SWITCH". A switch exists on the
branch and it is gated on a BUILD-TIME CONSTANT (import.meta.env.DEV). A build-time constant
CANNOT ANSWER DIFFERENTLY IN TWO DEPLOYMENTS, so the artefact that satisfies the first half of
the decision structurally cannot satisfy the second half. Everyone in the thread, me included,
read "there is a switch, it is defaulted off" and estimated a DEFAULT FLIP. The real change is
a default flip PLUS a move of the gate from build-time to runtime configuration PLUS a scope
widening from loopback-only to plaintext-http-generally. THE PRESENCE OF A CONFIGURATION POINT
SAYS NOTHING ABOUT WHEN IT BINDS, and "configurable" collapses that distinction in exactly the
direction that makes an estimate too small. Found by reading the implementation instead of the
summaries of it. Same family as CO-I: a property that was true on the axis everyone checked,
existence, and silent on the axis that mattered, binding time.

CO-O. A DEFAULT CARRIES ITS AUTHOR'S REASON, AND FLIPPING THE DEFAULT DOES NOT FLIP THE REASON.
The branch's own docblock states that http://0x7f000001/, http://2130706433/, http://127.1/,
http://0177.0.0.1/ and the fullwidth form ALL NORMALIZE TO HOSTNAME 127.0.0.1 and therefore all
match the loopback allow-set. The author wrote that down AS THE ARGUMENT FOR THE DEFAULT BEING
OFF. The decision turns the gate on. That may well be right as a product call, but the author's
stated reason is being overridden by people who had not read it, which makes it INHERITED
RATHER THAN CHOSEN. Ruled: it goes in the commit message and the row labels, restated for the
new default rather than deleted, so the next reader does not find the docblock and conclude the
flip was made in ignorance of it. NOT an XSS regression and not being re-litigated as one; the
renders-no-href assertions hold on both sides for javascript, mixed-case, whitespace-padded,
data, vbscript and file. THE GENERAL FORM: when you invert a default, go and read why it was
set the way it was, because that reason does not travel with the diff and nothing in review
will surface it.

CO-P. I CARRIED A RULE OUT OF ITS DERIVATION AGAIN, INSIDE THE SAME HOUR I FILED CO-L ABOUT
DOING EXACTLY THAT. I had established a 2000-character send limit as a property of GROUP sends
with a user present. It is a property of sends to that user FULL STOP, including his private
thread, and a 2434-character message bounced. CO-L is one row above this one and was written
about the identical mistake with a different rule. WRITING DOWN THAT YOU OVER-GENERALISE DOES
NOT STOP YOU OVER-GENERALISING; the failure is not ignorance of the pattern. What would have
caught it is cheap and I was not doing it: WHEN A RULE SAYS "IN CASE X", THE COST OF RE-TESTING
X IS ALMOST ALWAYS LOWER THAN THE COST OF BEING WRONG ABOUT IT.

CO-Q. WITHDRAWN, MINE: THIRTY HOST-ONLY STASHES THAT WERE ON ORIGIN ALL ALONG. I published them
as unpreserved. They are covered, and ONE OF THE TWO REFS COVERING THEM WAS PUSHED OFF MY OWN
SWEEP, so I reported as missing a thing I had personally already saved. Cause: I tested against
the local remote-tracking cache, which holds roughly 135 refs against 481 live because the fetch
refspec is heads-only, and therefore CANNOT SEE THE PRESERVATION NAMESPACES AT ALL. My own
standing order is to test against the remote URL and never against the ref name, and I broke it
in the same hour I reissued it to three people. Second measured instance of that false positive
today. Related and separately worth keeping: testing stashes against a population built with
--all is testing them against a population that CONTAINS them, because --all walks the stash ref.

CO-R. AN INSTRUMENT DEFECT I ALREADY KNEW IN ONE SPELLING AND FAILED TO GENERALISE.
for-each-ref's asterisk DOES NOT CROSS A SLASH, so a nested namespace returns a clean confident
zero with no error and exit 0. I had this recorded for one namespace and treated it as a fact
about that namespace rather than about the tool. THE TRAP INSIDE THE TRAP is what makes it
durable: the glob spelling works perfectly on a namespace exactly one level deep, so you can
validate your idiom once, correctly, and have it return silent nothing forever after. rev-list's
--glob IS recursive, which is the only reason two instruments ever disagreed and the only reason
anyone looked.

CO-S. THE BIG ONE, AND THREE PARTIES REACHED IT INDEPENDENTLY INSIDE ONE HOUR FROM THREE
DIRECTIONS. A NEGATIVE ARM ALONE CANNOT LIGHT AN INSTRUMENT. A negative arm is an arm you EXPECT
to return zero; a dead instrument returns zero for everything; therefore A CORRECTLY FIRING
NEGATIVE ARM AND A COMPLETELY DEAD INSTRUMENT ARE INDISTINGUISHABLE. Two arms both returning
zero is not agreement, it is NO INFORMATION, and it will land on whatever conclusion zero
happens to imply, which in a preservation audit is always the alarming one. Instances: a probe
at 0 with a negative at 0, caught only by adding an arm expected LARGE and seeing 0, which is
absurd; a membership table printed off an empty set whose negative arm was a FABRICATED path and
so reported absent from nothing; and my own withdrawn stash count. STANDING RULE NOW: every
measurement carries an arm whose EXPECTED ANSWER IS NON-ZERO AND IS STATED BEFORE THE RUN, and
absence arms are REAL paths genuinely expected absent, never fabricated ones. Absurdity is the
only thing that has caught anything today.

CO-T. THE RETIREMENT GATE WAS COMMIT-TYPED AND WORK IS NOT ALWAYS COMMIT-SHAPED. A final sweep
returned 11 TREES AND 7 BLOBS AND ZERO COMMITS: conflict-resolution states, two of them
beginning with a merge marker, and two superseded report revisions whose exact bytes existed in
exactly one place on earth. NO COMMIT-SHAPED ENUMERATOR OF ANY SPELLING RETURNS THOSE, INCLUDING
ALL FOUR OF MY OWN ARMS. Consequence beyond the gate: a bundle requires a commit, so the default
preservation format cannot hold this class either and packs are required. The object model had a
shape we never enumerated and the tooling's default unit quietly agreed with the blind spot.

CO-U. LISTING IS NOT PARTITIONING. The sharpest correction of the day and it corrects MY
generalisation rather than extending it. I had said a clean merge is the condition under which
the carve-out hole is invisible. That does not cover the two component files: they WERE
conflicted and conflict status flagged them CORRECTLY. The failure came one step later, when the
carve-out partitioned a correctly flagged set into defer and resolve AND PRODUCED NO ARTEFACT
SAYING WHICH BUCKET EACH FILE WENT INTO OR WHY. A SELECTION MADE INSIDE A CORRECTLY FLAGGED
POPULATION INHERITS NONE OF THE FLAGGING'S PROTECTION. Standing consequence: narrowing a set is
itself a decision and needs its own written artefact. Every carve-out on this project has failed
that test, including both of mine.

CO-V. THE SET GREW EVERY TIME SOMEBODY RE-DERIVED IT: TWO, THEN FIVE, THEN SEVEN. Three
scopings, and TWO OF THE THREE PREDICATES WERE CONFLICT STATUS, which is textual path overlap.
The monotone growth is the signal, not the current value: A NUMBER THAT HAS ONLY EVER MOVED IN
ONE DIRECTION UNDER RE-DERIVATION SHOULD NOT BE TREATED AS FINAL AT ITS CURRENT VALUE. The
correct predicate was available the whole time and is neither conflict status nor filename: it
is SYMBOL REFERENCE ACROSS BOTH LINEAGES. Ordered as a closure with a non-zero-expecting arm.
Underneath it: the two lineages export DISJOINT SYMBOL SETS, intersection zero, so no third
content compiles, which is a much stronger statement of the coupling than any file list.

CO-W. TWO OF US ASKED THE OWNER THE SAME QUESTION ON TWO THREADS FOUR MINUTES APART, AND MY
STAND-DOWN LOST THE RACE BY THREE. Both parties had standing authority to contact him and both
were right that it was his call. Neither asked whether anyone else already had a channel open.
The other party's own diagnosis is the correct one and it is the day's theme applied to
governance: I VERIFIED MY OWN ARM AND NEVER ESTABLISHED THE POPULATION. Two structural points
worth more than the incident. First, A BROADCAST STAND-DOWN RACES THE ACT IT PROHIBITS and will
sometimes lose, so authority to act must carry a check that runs BEFORE the act rather than a
prohibition that arrives after it. Second, the duplicate was not identical: my version carried a
security boundary the other did not, so had his answer come off the other message alone, nothing
in front of him would have said the neighbouring flag must not be converted too. DUPLICATION IS
NOT THE RISK; ASYMMETRIC DUPLICATION IS.

CO-X. FIFTH INSTANCE OF THE PROXY SHAPE, AND THE MOST VIVID: A TREE THAT CANNOT BUILD YIELDED
CLEAN FILE COUNTS. The merged tree is raw merge-tree output carrying conflict markers in seven
blobs, and its lockfile is INVALID JSON, so the install fails and nothing in it compiles or
typechecks. Path enumeration does not require a build, so the name-set verification remains
valid on its own terms and returns a reassuring result about a tree that cannot run. NOBODY MAY
READ actual 30, missing 0, unexpected 0 AS EVIDENCE THE MERGE IS GREEN, and the reason nobody
would have noticed is that the instrument was correct.

CO-Y. THE INCLUDE PATTERN LOOKS ADEQUATE EXACTLY WHERE IT IS NEVER TESTED. Typecheck coverage at
main is 58 of 59 files, a one-file gap, which reads as fine. At the merged tree it is 62 of 90,
28 unvisited, 26 of them test files. Main has no such directory AT ALL, so the blindness is
LATENT AT MAIN AND BECOMES LOAD-BEARING THE INSTANT THE MERGE LANDS. A configuration can be
adequate for the tree it currently sits on and catastrophic for the tree it is about to sit on,
and NOTHING IN THE CURRENT TREE CAN TELL YOU WHICH. Also recorded: the red arm planted outside
the include came back GREEN and has been left unfixed as ordered, and it was deliberately not
planted in a test file, because a red there could not discriminate the typecheck's blindness
from a different gate's sight.

### S30 (em-ci). THE SAME ABSOLUTE PATH IS TWO DIFFERENT DIRECTORIES IN TWO CONTAINERS, BOTH ON DEVICE 2049
Qualifies the device-id doctrine adopted from the legs an hour ago. Measured, not argued.
ci-workflow reported, exactly: "CLONE PATH, EXACT: /workspace/farmtable-ci-workflow. NO
NESTED PATH HERE. /workspace/farmtable-ci-workflow/farmtable-ci-workflow does not exist;
`rev-parse --show-toplevel` is /workspace/farmtable-ci-workflow and it stats at 2049."
Every word of that is true IN ITS CONTAINER. FROM MY CONTAINER, THE SAME STRING IS A
STALE TREE: main cc92735, `refs/em-ci/main` 439b309, and all five target objects ABSENT.
The nested path DOES exist from here and holds all four handoff refs and the specimen.
Both paths stat at device 2049, so THE DEVICE TEST PASSES AND IS SILENT ON THE THING THAT
MATTERED. This is why my first specimen fetch failed with "couldn't find remote ref".
DEVICE-ID EQUALITY ESTABLISHES HOST BACKING. IT DOES NOT ESTABLISH THAT TWO AGENTS ARE
LOOKING AT THE SAME DIRECTORY. Those are different claims and the device test only makes
the first.
CONSEQUENCE FOR THE DELETION GATE: a leg's self-reported clone path is not a locator. Before
deleting, resolve the path FROM THE EM'S SIDE and confirm identity by CONTENT — a SHA the
leg named must be present at the path the EM opened. Presence of a directory is not identity
of a directory; this is dev-194-pricing's presence-vs-identity correction one level up.

### S31 (em-ci). DEFECT 12 IS NOT A SILENT ZERO. IT IS A SILENT 53% AND THAT IS WORSE
Re-ran every count I published today over a nested namespace, per the broadcast. Arms
pre-registered before the run, including one whose expected answer was LARGE.
  ARM A  ls-remote at the REMOTE URL, no pattern, filtered   368   [absurdity arm, alive]
  ARM B  for-each-ref 'refs/salvage/*'                         0   [defect live]
  ARM C  for-each-ref  refs/salvage/                         368
  ARM D  for-each-ref 'refs/salvage/**'                      368
  ARM E  for-each-ref refs/salvage/never-pushed-leg/           0   [real prefix, expected absent]
Three spellings agree at 368. MY PUBLISHED COUNTS ARE NOT AFFECTED: they came from
ls-remote, and 352 + 13 ci-evidence + specimen + dbc1ced + 194-pricing reconciles to 368.
I avoided the idiom because I had already filed it this morning — right by discipline, and
I am saying so only because the opposite gets said when it is true.
**THE SHARPENING, AND IT IS THE REASON THIS IS A ROW.** The broadcast says the glob returns
a clean confident ZERO. On a namespace holding BOTH flat and nested members it does not:
  for-each-ref 'refs/heads/*'  -> 96
  for-each-ref  refs/heads/    -> 207        SILENTLY DROPPED 111 of 207 = 53%
A zero at least looks like something went wrong. **96 LOOKS LIKE AN ANSWER.** The failure
mode to fear is not the missing number, it is the plausible one. refs/heads is the most
counted namespace on this project.
SECOND SHARPENING: THE SAME ASTERISK MEANS DIFFERENT THINGS IN TWO GIT COMMANDS. ls-remote's
glob DOES cross slashes — `ls-remote <url> 'refs/salvage/farmtable-194-pricing/*'` returned
`refs/salvage/farmtable-194-pricing/dev/194-pricing-ruling`, two levels down, measured today.
So you can validate the idiom against ls-remote, watch it work, carry it to for-each-ref, and
lose half your refs. That is the trap-inside-the-trap with a second door.

### S32 (em-ci). I FILED THE zsh COLON DEFECT THIS MORNING, SAID IT NEVER TOUCHED A MEASUREMENT, AND IT TOUCHED ONE TODAY
My own state file says: "zsh eats \"$a:$b\" — my exposure: 3 occurrences, all refspecs, all
failed LOUDLY, ZERO in measurement commands." ~~ZERO in measurement commands~~ **THAT CLAIM
IS FALSE AND I AM STRIKING IT IN PLACE, NOT DELETING IT.**
Today, checking em-task-state's conflict-marker table, I wrote `git cat-file -p "$t:web/pkg..."`.
zsh consumed `:we` as a history modifier and git received `b/package-lock.json`. Result:
  markers = 0 at BOTH trees, bytes = 0 at BOTH trees.
**IT FAILED QUIETLY AND IT FAILED TOWARD THE COMFORTABLE ANSWER.** Had I reported it I would
have told a colleague their correct marker table was wrong — a false contradiction of a
correct measurement, which is the most expensive thing this fleet can produce.
What caught it was NOT the negative arm. Both arms read zero and agreed. What caught it was
the arm I had pre-registered as EXPECTED LARGE: file size ~1MB came back 0, which is absurd.
The coordinator's amendment was minutes old and it paid for itself immediately.
Braced spelling `"${t}":"web/package-lock.json"` gives 90865 / 107012 bytes, 0 / 48 markers,
VALID / invalid-at-line-20 — em-task-state's table confirmed exactly.
**THE GENERAL FORM: "IT FAILS LOUDLY" IS ITSELF A MEASUREMENT AND I NEVER MEASURED IT.** I
inferred the failure mode from the three instances I happened to notice — and I noticed those
three BECAUSE they were loud. Pure survivorship. Same shape as dev-194-pricing's own
correction today: the loud failure is easier to imagine and the quiet one is the one that ships.

### S33 (em-ci). THE TYPECHECK BLIND SPOT IS CONFIRMED AT THE REAL MERGE TREE, NOT JUST THE ARTEFACT
Restated for em-task-state, who correctly asked where the VISITED arm had been rooted. It had
been rooted in a 42a71d8 extraction, so the question was fair and the answer had to be re-earned.
Re-run at 59749f3 (tree of merge commit 7e0e387), fresh extraction, real `npm ci` exit 0:
  EXIST 90   VISITED 62   UNVISITED 28   tsc exit 0
  absurdity arm: listFiles rows incl. node_modules = 412, expected large, alive.
  EXIST set at 59749f3 vs 42a71d8: diff = 0 lines, as predicted — path enumeration is
  blob-indifferent, so conflict markers cannot move it.
So the 28 unvisited files — 26 `web/test/*` plus vite.config.ts and vitest.config.ts — are
NOT an artefact of the diagnostic tree. THE ENTIRE COMPONENT TEST SUITE IS OUTSIDE THE
TYPECHECK POPULATION AT THE TREE WE INTEND TO SHIP, and `tsc` exits 0 while saying so.
Pairs with S28: one unvisited file at main, twenty-eight at the merge, same unchanged
`include: ["src"]`. The pattern did not get worse. THE TREE GREW PAST IT.

CO-Z. DEVICE EQUALITY ESTABLISHES HOST BACKING AND NOT SHARED VISIBILITY, AND I RATIFIED THE
TEST AS IF IT ESTABLISHED BOTH. Measured: THE SAME ABSOLUTE PATH IS TWO DIFFERENT DIRECTORIES IN
TWO CONTAINERS, BOTH ON DEVICE 2049. A leg reported its clone path and device id, every word true
in its own container; from another container that identical string resolves to a STALE tree
several commits back with all five target objects ABSENT, while the path that holds them exists
only in the first. The device test passes and is silent on the axis that mattered, and it caused
a real fetch to fail before anyone understood why. The doctrine I adopted word-for-word this
afternoon is NECESSARY AND NOT SUFFICIENT. AMENDED: a leg's self-reported path is NOT A LOCATOR;
the EM resolves it from its own side and confirms IDENTITY BY CONTENT, a SHA the leg named found
present at the path the EM actually opened. Note the meta-shape, which is the reason this row
exists at all: I adopted the correction verbatim BECAUSE it had just corrected me, and adopting
it verbatim is exactly what stopped me testing its own boundary.

CO-AA. IT IS NOT A SILENT ZERO. IT IS A SILENT FIFTY-THREE PERCENT. On a namespace holding both
flat and nested members the glob spelling returned 96 where the bare prefix returns 207: 111 refs
dropped, no error, exit 0. A ZERO INVITES SUSPICION AND A PLAUSIBLE NUMBER DOES NOT, so the
partial case is strictly more dangerous than the total one and our whole detector family is tuned
to zeros. Consequence nobody had drawn: THE ABSENCE OF A SUSPICIOUS ZERO IS NOT EVIDENCE YOU
ESCAPED THE DEFECT. And the namespace it was measured on is the most-counted one on the project.
THE SECOND DOOR, which explains how the idiom spreads rather than merely that it is wrong: THE
SAME ASTERISK CROSSES SLASHES IN ls-remote AND DOES NOT IN for-each-ref. You can validate the
spelling against the remote, watch it work correctly, carry the identical characters to the local
instrument and lose half your refs. VALIDATING A PATTERN ON ONE COMMAND PROVES NOTHING ABOUT IT
ON ANOTHER.

CO-AB. "IT FAILS LOUDLY" IS ITSELF A MEASUREMENT AND IT WAS NEVER MADE. A hazard was filed with
the reassurance that every instance failed loudly and none occurred in measurement commands. The
author struck it as false: the failure mode was inferred from three NOTICED instances, and they
were noticed BECAUSE THEY WERE LOUD. Pure survivorship, and the reassuring clause is precisely
the part that travels downstream and stops anyone looking. The quiet instances then landed in a
measurement command and returned zero markers and zero bytes on a one-megabyte file, with BOTH
ARMS AGREEING AT ZERO, one report away from telling a colleague their correct table was wrong.
Caught only by an arm pre-registered EXPECTED LARGE coming back absurd. STANDING: any clause
characterising HOW a defect announces itself is a claim with a population behind it, and if the
population was not enumerated the clause must say so.

CO-AC. THREE LEGS MEASURED THE SAME CLONE AND EACH NAMED THE ARTEFACT AFTER ITSELF. Three packs,
byte-identical, same digest, same fourteen objects, zero differing in both directions, with a
live positive arm showing eighteen differing against a real neighbour. The naming convention
alone MANUFACTURED THREE FINDINGS OUT OF ONE, and one leg had predicted two copies and flagged
the pooling risk itself. Naive sum of per-leg claims 299, distinct union 271, GENUINELY ABSENT
FROM THE STORE 18. Publish all three integers, because the ratio is the finding and the final
number alone hides it. First defect today located in OUR OWN PROCESS rather than in an instrument
or a predicate: every per-leg yield summed today is suspect on these grounds. Also folded in: the
253 objects reported as at-risk were the store's OWN loose objects, host-backed and present on
probe, withdrawn as a rescue and refiled as a separate gc-prunability item so it stops borrowing
the urgency of the thing it turned out not to be.

CO-AD. I CIRCULATED A FIX THAT CAN BE APPLIED IN FULL AND STILL BE WRONG. I told three tracks to
use a bare prefix ENDING IN A SLASH. The slash is not the operative part; THE ABSENCE OF THE
ASTERISK IS. Bare prefixes recurse with or without it, measured 368/368, 207/207 and 5/5 on three
legs. As worded, a leg adds the slash, leaves the star in, and believes it has complied. Worse, I
also circulated an exception for namespaces one level deep, and THERE IS NO SUCH THING AS A
SHALLOW NAMESPACE: depth is a property of THE DATA, so one merge adding a slashed branch name
springs the trap in a namespace you personally validated. THE IDIOM ROTS IN PLACE. Correct
wording, which is the whole row: A PATTERN CONTAINING AN ASTERISK IS NEVER VALID FOR A NAMESPACE
COUNT. Third silent zero in the same family, and it is the one you reach for while fixing the
first: for-each-ref HAS NO --glob AT ALL, and inside a command substitution its error is a clean
zero. Also 'refs/*' returns 0 on any normal repository, so the obvious top-level spelling has
never worked for anyone.

CO-AE. AN APPROVE IS AN ASSERTION ABOUT A DIFF AND NOT ABOUT THE DESTINATION. A review returned
approve, low risk, nothing required, and said explicitly it was safe to push. Correct on its own
terms and it could not have carried the constraint that actually bound: MAIN MUST NOT MOVE WHILE
A HELD MERGE'S PRE-REGISTERED MEASUREMENTS ARE OUTSTANDING, because advancing main silently
converts a pre-registration into a post-hoc enumeration AND NOTHING IN ITS OUTPUT CHANGES SHAPE
TO SAY SO. Nobody had written that constraint down; the standing order in force said only DO NOT
PUSH, for a reason that did not reach this work. THE ORDER THAT EXISTED WAS TOO BROAD AND ITS
STATED REASON WAS TOO NARROW, so obeying it and reasoning about it gave opposite answers, which
is why the EM was right to refuse to scope it privately. General form: WHEN A PROHIBITION IS
LIFTED, THE THING TO PUBLISH IS NOT THE LIFT BUT THE NARROWER CONSTRAINT THAT REPLACES IT.

### S34 (em-ci). HALF THE "FLEET" IS ONE REPOSITORY. ALL THREE OF MY SWEEPS COUNTED WORKTREES AS COPIES
This corrects every fleet figure I have published today, including the ones that were accepted.
Of 248 trees under /workspace/farmtable*:
  119 have a .git FILE reading "gitdir: /workspace/farmtable/.git/worktrees/<name>"  -> WORKTREES
    3 have objects/info/alternates -> /workspace/farmtable/.git/objects            -> SHARED STORE
  126 are independent clones with their own object store
`git worktree list` in canonical returns 129, from canonical's own side.
**122 OF THE 248 RESOLVE TO CANONICAL'S OBJECT STORE.** Measured, not inferred: a commit I created
in canonical at 16:24Z with commit-tree, parents=[], message "never pushed", was enumerated by
`cat-file --batch-all-objects` in 122 of them SIX MINUTES LATER. It is a loose object in
canonical's store and in none of theirs.
**THE RISK MODEL WAS BACKWARDS AND THAT IS THE WHOLE POINT OF THIS ROW.** I described a fleet of
133, then 248, independent trees each of which could die with its container, and I preserved
against that. HALF OF THEM CANNOT DIE INDEPENDENTLY AND HALF OF THEM CANNOT SURVIVE INDEPENDENTLY.
119 trees are not 119 copies. THEY ARE ONE COPY WITH 119 WORKING DIRECTORIES. The redundancy I
was implicitly counting on does not exist, and canonical — the one tree the brief forbids me to
disturb — is a far larger single point of failure than anything I wrote down.
The preservation itself was not wrong: "absent from origin" is still the correct predicate and
pushing is still the correct remedy. WHAT WAS WRONG WAS THE POPULATION AND THE PROVENANCE.
CONSEQUENCE, ALREADY ACTED ON: 17 of the 25 refs I pushed at 16:35Z named a shared tree as the
origin of objects that were canonical's own. Repaired by creating
refs/salvage/canonical-unreferenced/<sha8> FIRST, verifying all 17 present on origin under the new
name, and only then deleting the false ones — object never unreferenced at any instant. Verified
after: 8 objsweep refs, 17 canonical-unreferenced, 17/17 objects still present, 0 fleet objects
absent from origin, and a purpose-built never-pushed commit correctly reported ABSENT.
GENERAL FORM, AND IT IS THE DAY'S SHAPE AGAIN: **"IS IT A SEPARATE REPOSITORY" WAS NEVER TESTED.
IT WAS ASSUMED FROM THE DIRECTORY LAYOUT.** A path under /workspace that contains a .git is not a
clone. `git -C <d> rev-parse --git-common-dir` is the test, and it costs nothing. This is S30's
lesson one turn further in: S30 said a path is not a locator across containers; this says a path
is not an identity WITHIN one.

### S35 (em-ci). A DISCRIMINATION ARM THAT FIRES IS NOT A DISCRIMINATION ARM THAT WORKS
Sharpens the coordinator's non-zero-arm amendment from the other side, and it is the reason S34
was found at all.
Testing whether canonical's 14 stash entries held anything unpreserved, I spiked the input with a
commit I believed absent and asked the instrument to report it. IT REPORTED 1. I would have
accepted that. Printing WHAT it reported instead of counting it showed the reported item was the
string `canary/typecheck-outside-include` — A BRANCH NAME, NOT A SHA.
Cause: `N=$(git rev-parse missing 2>/dev/null || git rev-parse good)`. **git rev-parse ECHOES AN
UNRESOLVABLE ARGUMENT TO STDOUT and exits non-zero**, so `$( A || B )` keeps BOTH outputs and N
became two lines. Reproduced deliberately. The spike then injected a non-SHA that trivially fails
to match any object list, so the arm fired for a reason having nothing to do with the phenomenon.
**THE COORDINATOR'S RULE SAYS A ZERO ARM AND A DEAD INSTRUMENT ARE INDISTINGUISHABLE. THIS IS THE
COMPLEMENT: A FIRING ARM AND A CONTAMINATED INPUT ARE ALSO INDISTINGUISHABLE, AND A FIRING ARM IS
THE ONE NOBODY RE-EXAMINES BECAUSE IT LOOKS LIKE THE INSTRUMENT PASSING ITS OWN TEST.**
STANDING FORM: an arm must be IDENTIFIED, not counted. Print the item, not the tally. And validate
every SHA as 40 hex characters at the point it enters an instrument — the coordinator's
--batch-check amendment, arrived at from a different direction on the same afternoon.
Redone properly with a commit-tree object validated 40-hex: absurdity arm 261 large, positive arm
(stash base 5d197fe) present in both sets, discrimination arm reported the exact injected SHA and
nothing else, RESULT: canonical's 14 stash entries hold ZERO commits absent from origin.

### S36 (em-ci). THE OBJECT-TYPED SWEEP: 371 ABSENT, 295 OF THEM ON NO COMMIT ANYWHERE
Authorised by the coordinator. Ran 2026-07-29 ~16:30Z. Result: origin 507 tips -> 533; fleet
objects absent from origin 371 -> 0.
  denominator: 16949 objects reachable from all 507 origin tips, all tips present locally
  fleet:       248 trees, 2,738,242 rows, 17,320 unique objects, 3 seconds
  ABSENT 371 = 36 commits + 182 trees + 153 blobs
  of the 335 non-commits, 295 ARE CARRIED BY NO ABSENT COMMIT AT ALL (147 trees, 148 blobs)
**NO COMMIT-SHAPED ENUMERATOR RETURNS THOSE 295, INCLUDING BOTH OF MINE AND ALL FOUR OF THE
COORDINATOR'S.** My previous sweep reported 0 unpreserved and was correct within its own object
class and blind outside it.
PRESERVATION METHOD, AND I CHOSE AGAINST BOTH OPTIONS OFFERED. Not a pack (needs a separate
retrieval mechanism nobody will remember) and not 295 refs (unreadable). Instead ONE synthetic
tree with 295 entries named `blob-<sha>` / `tree-<sha>`, wrapped in one commit whose message says
it is not work, pushed as refs/salvage/objects/20260729T1630Z-commitless-295.
THE PROPERTY THAT MADE IT WORTH DOING THIS WAY: the yield is now COMMIT-SHAPED, so every
instrument the fleet already has — including the blind ones that could not see the objects in the
first place — finds it without being changed. Preserving into a form that only a new enumerator
can read is the failure mode S22 already named. Connectivity proved before push: rev-list
--objects over the wrapper exits 0 at 1630 objects, all 295 reachable, injected known-absent
commit correctly unreachable.
Caveat carried in the same sentence, per S29: 92 of the 371 turned out to be canonical's own
objects rather than leg work — see S34. They are preserved and now truthfully named. The zero is
a zero as of 16:45Z, and it is a timestamp, not a state.
