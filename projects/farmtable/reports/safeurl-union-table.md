...
# THE 91-ROW TABLE — RE-RUN, CONFIG-CORRECT, EVERY DENOMINATOR NAMED

Supersedes the 81-row arms. Both arms re-run from scratch against
`rows/union91.json`. Nothing carried forward by hand.

## 81 + 10 = 91, and the catch that proves the count is two-sided

The 10 added, all from `ffe965d`, none previously held:

| id | Input | BRANCH asserts | Arm |
|---|---|---|---|
| 82 | `https://example.com/a` | accept, identical | CARVEOUT |
| 83 | `https://github.com/acme/repo/issues/7` | accept, identical | CARVEOUT |
| 84 | `http://localhost:3000/a` | **accept** | CARVEOUT |
| 85 | `http://127.0.0.1:3000/a` | **accept** | CARVEOUT |
| 86 | `http://example.com/a` | reject | CARVEOUT |
| 87 | `" javascript:alert(1) "` | reject | CARVEOUT |
| 88 | `HTTPS://Example.COM/a` | accept → `https://example.com/a` | CARVEOUT |
| 89 | `"  https://example.com/a  "` | accept → `https://example.com/a` | CARVEOUT |
| 90 | `https://example.com:443/a` | accept → `https://example.com/a` | CARVEOUT |
| 91 | `https://example.com/a/../b` | accept → `https://example.com/b` | CARVEOUT |

**Kept visible on instruction:** I first computed **11** missing and found
`undefined` was already present as row 31 under a `__UNDEFINED__` sentinel my
comparison had stringified to a key that could never match. Corrected to 10
before publishing. Left in because it is the evidence that the counting is now
checked in both directions rather than merely larger.

## The three integers, restated with units and sources

- **MAIN: 49 distinct inputs.** `c3e1b5c` + fixture `4a54328`. No configuration
  switch exists in `659ef58`, so MAIN has exactly one arm.
- **BRANCH: 55 distinct inputs** across two files and **two arms** —
  **BLOCKING 45** (`a9e49ff`, plain Node, `import.meta.env` undefined) and
  **CARVEOUT 21** (`ffe965d`, Vitest, `import.meta.env.DEV` true), **11 inputs
  asserted under both**. 45 + 21 − 11 = 55.
- **UNION: 91 distinct inputs.** 49 + 55 − **13 shared** = 91. Of the 13 shared,
  **12** are identical (input, verdict, value) triples; the thirteenth is
  `http://[::1]/x`, contradiction C1, deliberately not deduplicated.

## ARM A — every control green, which is the point

| Subject | Scored against | Result |
|---|---|---|
| `MAIN.pristine` | MAIN's 49 | **49/49 pass, 0/49 fail** |
| `BRANCH.blocking` | BRANCH BLOCKING's 45 | **45/45 pass, 0/45 fail** |
| `BRANCH.carveout` | BRANCH CARVEOUT's 21 | **21/21 pass, 0/21 fail** |

### THE RETRACTION, MEASURED

> ~~"`BRANCH.carveout` fails **8 of its OWN rows** (64–71)."~~

**`BRANCH.carveout` passes 21/21 of its own rows.** The earlier figure was
produced entirely by scoring `a9e49ff`'s **BLOCKING** rows against the
**CARVEOUT** build — a configuration `ffe965d`'s docblock explicitly excludes:
...
Quoted next to the strike so the next reader can see why the original misreading
was available: from `a9e49ff` alone the config split is invisible, because the
split is expressed in the **runner**, not in the assertions.

**Credit where ruling 4 is concerned: BRANCH had already done the config
labelling, by file, before the ruling asked for it.**

### Cross-side conformance

| Subject | Scored against | Result |
|---|---|---|
| `MAIN.pristine` | BRANCH BLOCKING's 45 | 22/45 pass, **23/45 fail** — 50, 58–76, 78, 79, 80 |
| `MAIN.pristine` | BRANCH CARVEOUT's 21 | 15/21 pass, **6/21 fail** — 80, 86, 88, 89, 90, 91 |
| `BRANCH.blocking` | MAIN's 49 | 39/49 pass, **10/49 fail** — 43, 44, 47, 49–54, 56 |
| `BRANCH.carveout` | MAIN's 49 | 39/49 pass, **10/49 fail** — identical set |

Each side still fails rows the other passes. **The table did not choose a
winner** and the 10 extra rows did not make it choose one. Of MAIN's 6 failures
against the CARVEOUT arm, **four (88, 89, 90, 91) are the return contract** and
one (86) is C2 — so the extension sharpened the contract question rather than
adding a new axis.

### An apparent contradiction that the config labels dissolve

| | Input | Verdict | Arm |
|---|---|---|---|
| id 64 | `http://localhost:8080/tasks/1` | **reject** | BLOCKING |
| id 84 | `http://localhost:3000/a` | **accept** | CARVEOUT |
| id 65 | `http://127.0.0.1:3000/tasks/1` | **reject** | BLOCKING |
| id 85 | `http://127.0.0.1:3000/a` | **accept** | CARVEOUT |

Loopback http, rejected and accepted by the same side. **Not a contradiction —
one policy under two configurations.** Machine-checked: of the 11 inputs asserted
under *both* arms, **0 disagree**. BRANCH is internally consistent.

Stripped of configuration labels this reads as a side contradicting itself, which
is precisely the false finding I filed an hour ago. **An unlabelled row is not a
result** — demonstrated, not asserted.

## ARM B — kill power, denominators in the sentence

| Mutant | Scored against | Result |
|---|---|---|
| `MAIN.disarmed` (`numstat 1 63`) | MAIN's 49 | **31/49 KILL, 18/49 VACUOUS** |
| `BRANCH.disarmed` (`numstat 1 25`) | BLOCKING's 45 | **41/45 KILL, 4/45 VACUOUS** |
| `BRANCH.disarmed` | CARVEOUT's 21 | **15/21 KILL, 6/21 VACUOUS** |
| `BRANCH.disarmed` | all 55 BRANCH inputs | **47/55 KILL, 8/55 VACUOUS** — 30, 31, 77, 81, 82, 83, 84, 85 |

**MAIN 31/49. BRANCH 47/55.** The disarm is config-independent — it replaces the
whole body — so it is legitimate to score it across both arms.

### The 10 new rows are a clean out-of-sample test of the Arm B finding

**6 of 10 KILL the disarm (86, 87, 88, 89, 90, 91); 4 of 10 are vacuous (82, 83,
84, 85).**

The four vacuous ones are accepts whose input is *already normalised*. The four
normalisation rows — 88, 89, 90, 91 — all kill. **This is the structural claim
reproducing on rows the table had never seen**: an accept that asserts an exact
*transformed* value kills a passthrough; an accept that asserts the input back
cannot.

And these four rows are not an accident of my sampling. `ffe965d`'s docblock says
why they exist:

> "Returning `raw` instead **survived the round-2 mutation run** because every
> other case in this file happens to be already-normalized. **These inputs are
> not.**"

The branch author ran this arm, found this survivor, and added exactly these rows
to kill it. My measurement now confirms their stated intent on their own rows.
**The finding stands; my claim to have discovered it does not.**

## Config label coverage

Machine-checked: **0 rows of 91 carry no configuration label.** Every row states
the arm it was measured under; MAIN's 49 are labelled single-arm because `659ef58`
contains no switch to vary.

---

# RE-BASE TO `e64138c` — steps 1–3

Ordered 16:06Z, superseded 16:07Z and again 16:09Z. The `633f8f2` base ruling is
withdrawn by its author. Everything below is measured from my own clone; the
coordinator's table was not taken on trust.

## 1. Lineage — verified, not accepted

`633f8f2` was not in my clone's reachable set and `e64138c` was not in it at all;
it does not exist in **any** `refs/heads` in canonical. It lives only in salvage
namespaces:

```
refs/salvage/dev-p2-rebase/p2-land          e64138c058ad707d2b08b3a213cfa63c17c8e953
refs/em-audit/salvage/dev-p2-rebase/p2-land e64138c058ad707d2b08b3a213cfa63c17c8e953
  ... and 6 further mirrors in em-net/em-net5..8/netcheck
```

Fetched from the **local path**, never the network remote. Every claim re-measured
with its rc printed:

| assertion | result |
|---|---|
| `aa08f1a` ancestor-of `e64138c` | rc=**0** (yes) |
| `633f8f2` ancestor-of `e64138c` | rc=**1** (no) |
| `merge-base(aa08f1a, e64138c)` | `aa08f1ae8ca972f463215f76113c121c4578ce70` |
| `merge-base(439b309, e64138c)` | `aa08f1ae8ca972f463215f76113c121c4578ce70` |
| `merge-base(633f8f2, e64138c)` | `7a0f220dbd9332cb8db62138c841777432b4eda4` |
| NEGATIVE ARM: `e64138c` ancestor-of `633f8f2` | rc=**1** (fired; direction not inverted) |

`aa08f1a` is therefore the genuine merge base of the main-side `439b309` and the
branch-side `e64138c`. `633f8f2` forks at `7a0f220` and is an ancestor of nothing
current.

## 2. The five blobs — confirmed independently

| path | `e64138c` | `633f8f2` | `439b309` (MAIN) | `aa08f1a` (merge base) |
|---|---|---|---|---|
| `web/src/util/safe-url.ts` | `d85bb5b` | `d85bb5b` | `659ef58` | **ABSENT** |
| `web/src/util/safe-url.test.ts` | `a9e49ff` | `a9e49ff` | `c3e1b5c` | **ABSENT** |
| `web/test/ft-inspector-code.safe-url.test.ts` | `869cb3c` | `869cb3c` | ABSENT | **ABSENT** |
| `web/test/ft-inspector-meta.safe-url.test.ts` | `0a85925` | `0a85925` | ABSENT | **ABSENT** |
| `web/test/safe-url.contract.test.ts` | `9391a2a` | `ffe965d` | ABSENT | **ABSENT** |

Four of five byte-identical between `e64138c` and `633f8f2`, as stated. The added
column is mine: **none of the five files exist at `aa08f1a`.** The true merge base
offers nothing to build on, only to write — which is why the choice of starting
tree is a policy choice and cannot be dodged by "just use the merge base".

## 3. The nine added lines — classified

`git diff --numstat 633f8f2 e64138c -- web/test/safe-url.contract.test.ts` → **9 0**.
Negative arm: the same command on `web/src/util/safe-url.ts` returns **0 rows**.
Sizes confirm: `ffe965d` 4,812 B / 99 lines → `9391a2a` 5,469 B / 108 lines.

**6 comment lines, 3 input rows, 0 assertion-only lines.**

The three inputs, extracted by pattern rather than by eye:
...
Asserted-case-row count, by pattern: `ffe965d` **21** → `9391a2a` **24**.
Negative arm: the same pattern against impl blob `659ef58` returns **0**.

### The delta from 91 is ZERO, and the reason matters more than the number

All three inputs were **already in the union** as rows **72, 74, 75**, sourced from
`a9e49ff`. So:

> **DENOMINATOR: 91 → 91. UNCHANGED.** The nine lines add no new inputs.

What they change is **configuration coverage**, which is exactly the axis the
coordinator ordered labelled:

| | before (`ffe965d`) | after (`9391a2a`) |
|---|---|---|
| rows in BRANCH **CARVEOUT** arm | 21 | **24** |
| rows asserted under **both** branch arms | 11 | **14** |
| row ids in both arms | 1, 5, 15, 18, 21, 28, 30, 31, 33, 34, 80 | + **72, 74, 75** |

`9391a2a` ports three inputs from the plain-Node suite into the jsdom contract
table. The blob's own added comment says why, and it is a coverage-topology
argument, not a new-case argument:

> "The check lived at `safe-url.ts:63` guarded only by the plain-Node suite —
> deleting it left this contract table fully green, so if the Node runner is ever
> retired in favour of this harness the defence loses all coverage."

That is the same defect class as my correction 4, written down by the branch author
before I made it: **a control that exists under one configuration and not another
is not a control.** Counting inputs would have shown nothing here; counting
(input × configuration) pairs is what shows it.

## 4. Conformance re-run — every control clean

Provenance gate first: `main.pristine` hashes to `659ef58`, `branch.pristine` to
`d85bb5b`. Both match their named blobs, so the run is admissible.

| subject | config | result |
|---|---|---|
| `MAIN.pristine` | single arm (`659ef58` has no switch) | **49/49 pass** |
| `BRANCH.blocking` | `d85bb5b`, plain Node, flag false | **45/45 pass** |
| `BRANCH.carveout` | `d85bb5b`, Vitest `import.meta.env.DEV=true` | **24/24 pass** |

The three promoted rows pass under the carve-out build, as predicted — the userinfo
check at line 63 is outside the loopback conjunction, so opening the flag does not
open userinfo.

### The promoted rows carry a finding against my own disclosed side

| row | input | MAIN | BRANCH.blocking | BRANCH.carveout |
|---|---|---|---|---|
| 72 | `https://user:pass@evil.example/` | **ACCEPT** | REJECT | REJECT |
| 74 | `https://github.com@evil.example/` | **ACCEPT** | REJECT | REJECT |
| 75 | `https://:pass@evil.example/` | **ACCEPT** | REJECT | REJECT |

MAIN accepts all three. Row 74 is the destination-confusion case: both call sites
render **static** link text, so `https://github.com@evil.example/` displays as
github.com while navigating to evil.example. This is the C3 ruling (userinfo →
reject) and **MAIN — my own track's work — is on the wrong side of it under every
configuration.** Recorded plainly because I am a disclosed party to that side.

## 5. Resulting-blob disclosure, per the binding constraint

For each of the five files, what my tree currently holds and whether it matches
MAIN, the BRANCH, or neither:

| path | my blob | matches |
|---|---|---|
| `web/src/util/safe-url.ts` | `d85bb5b` | **BRANCH** (inherited) |
| `web/src/util/safe-url.test.ts` | `a9e49ff` | **BRANCH** (inherited) |
| `web/test/ft-inspector-code.safe-url.test.ts` | `869cb3c` | **BRANCH** (inherited) |
| `web/test/ft-inspector-meta.safe-url.test.ts` | `0a85925` | **BRANCH** (inherited) |
| `web/test/safe-url.contract.test.ts` | `9391a2a` | **BRANCH** (inherited) |

**Five of five match the branch, and all five are INHERITED, not chosen.** I was
halted before step 3, so I have changed none of them. This is precisely the state
the coordinator warned about — a starting tree carrying a policy in its blobs — and
it is recorded as inheritance so that nobody later reads it as adjudication. Nothing
in this table represents a decision by me.

Two files in my tree are mine and match neither side, because neither side has them:

| path | blob | matches |
|---|---|---|
| `web/src/components/inspector/deactivated-link.ts` | `3aa63c6` | **NEITHER** (new) |
| `web/test/deactivated-link.test.ts` | `9330e01` | **NEITHER** (new) |

## 6. The fixture differential pin — measured, and larger than a file list

First correction to the pin as stated: **`4a54328` is a blob, not a commit.**
`git rev-parse 4a54328^{commit}` returns *"expected commit type, but the object
dereferences to blob type"*. It is `439b309:testdata/url-scheme-cases.json`,
12,359 B — confirmed by resolving the path and comparing.

Second: I nearly reported that `internal/server/convert.go` was **not** part of
the pin. My `git grep` used the exported spelling `ValidateURLField` against a
lowercase identifier, so it missed `convert.go:557`
`if err := validateURLField("remote_url", remoteURL); err == nil {`. A
case-insensitive sweep found it. **The pin was right and my sweep was wrong** —
recorded because a case-sensitive grep returns a confident empty result, the same
failure shape as defect 12.

### Where the pin's artefacts actually live

| path | `aa08f1a` (merge base) | `439b309` (MAIN) | `e64138c` (BRANCH) |
|---|---|---|---|
| `internal/server/urlvalidate.go` | ABSENT | `2fd421c` | ABSENT |
| `internal/server/urlvalidate_differential_test.go` | ABSENT | `0769838` | ABSENT |
| `testdata/url-scheme-cases.json` | ABSENT | `4a54328` | ABSENT |
| `internal/server/convert.go` | `e7df15f` | `bd7f6f0` | **`e7df15f`** |

Positive control: `go.mod` resolves to `99d3197` at all three, so the instrument
is not answering ABSENT to everything.

A case-insensitive sweep for `validateurlfield` across `e64138c` returns
**nothing**. There is no Go-side URL validator on the branch tip at all.

### The mirror image of the carve-out defect, and it points at MAIN

The coordinator's finding was that three branch-side test files escaped the p2
carve-out because they were pure additions and therefore never conflicted —
**conflict status is textual path overlap, not semantic coupling.** The same
property holds in the other direction and nobody has stated it:

- `urlvalidate.go`, the differential test and the fixture are **absent at the
  merge base and absent at the branch tip**. They are pure MAIN-side additions.
- `convert.go` at `e64138c` is **byte-identical to the merge base** (`e7df15f`).
  The branch never touched it; only MAIN did, to add the call site.

So under a genuine three-way merge all four come across unconditionally and
nothing is lost. **But if anyone implements "base `e64138c`" as "check out
`e64138c`'s tree and work there", the entire Go-side validator, its 42-case
fixture, the cross-language differential test and MAIN's `convert.go` call site
disappear silently** — no conflict, no marker, and not one of the five named
policy files would reveal it.

This favours MAIN, which is my own disclosed side, so it rests entirely on blob
identity above rather than on judgement.

### What the fixture actually pins

42 cases; server accept 11 / reject 31; client accept 16 / reject 26; **9 recorded
divergences**. It is read by *both* `urlvalidate_differential_test.go` (server
column, against `validateURLField`) and MAIN's `web/src/util/safe-url.test.ts`
(client column, against `safeHref`). Its own README states the point:

> "The SCHEME SETS agree. The DECISIONS do not: 9 of these 42 inputs are decided
> differently." … "PINNED AND SAMPLED — NOT BOUNDED."

The test carries three anti-vacuity guards of its own — it fails if the fixture
has no cases, if a divergence carries no note, if two notes are verbatim
identical, and if **no divergences remain at all**.

**Consequence for the merge that nobody has costed:** MAIN's `c3e1b5c` is the only
consumer of the client column. Adopting the branch's `a9e49ff` in its place leaves
the server column asserted by Go and **the client column asserted by nobody** —
the fixture half-dies, and no Go test can detect it, because all four guards run
server-side and still pass.

### The rulings measured against the fixture

| axis | rows touched | verdict |
|---|---|---|
| plaintext-http default ALLOW | 8 of 42 become **config-dependent** | **0 rewrites needed** — every recorded client value already equals the switch-OFF verdict |
| C3 userinfo → reject | 1 row: `https://user:pass@example.com/x` | recorded **server=accept, client=accept** — the ruling flips **both** columns |
| C1 reject `[::1]` | `http://[::1]/x`, recorded accept/accept | correct at default; rejects only with the switch ON. **No change at default** |

Two things follow:

1. **The C3 row requires a Go-side change to `validateURLField`, which nobody has
   scoped.** The ruling is not client-only.
2. If the client column is flipped alone, the fixture gains a **10th divergence**,
   and `TestSharedFixturesRecordRealDivergences` demands a unique non-empty note
   for it. That tripwire is designed and it will fire — correctly. It is the
   cheapest available detector that a client-side ruling has server-side reach.

### The gap the pin does not yet cover

**The fixture has no configuration column, and 8 of its 42 rows become
config-dependent under the ruled policy.** Carried forward unchanged it will
silently encode one configuration as if it were the whole answer — precisely the
defect the union table was rebuilt from 81 to 91 rows to eliminate. The fixture
needs a configuration axis before it can pin a switchable policy.
