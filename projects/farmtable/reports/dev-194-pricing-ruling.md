...
## §14. THE BASELINE-RED GAP, CLOSED — AND THE DEFECT IS BIGGER THAN §13 SAID

EM ruling 16:04:52Z declined both (A) and (B) and ordered the measurement named
in my own item 2 first, on the grounds that **(B) run blind would have produced
an oracle that reds on nothing real** — a guard whose failure mode is that it can
never fire, which is the class this issue exists to remove. That was the right
call and it is not the call I recommended.

Pre-registered results-free at `754dc16`, both branches named in advance,
prediction recorded (ALLOWED → (B)). Oracle shipped at `9cc9430`.

### THE CHARACTERIZATION, DERIVED NOT SAMPLED (mechanism read at `2ffc22a`)

`SameStageSet` is **positional** (`len`, then `a[i] != b[i]`). The r11 AFTER
endpoint is `unionStages(narrowAfter, wideAfter)`, which **preserves primary
order and APPENDS** what only the wide view names. `AllTerminalLabelStages`
returns its stages **sorted**. Therefore r11 **permits** a masked departure iff

```
narrowAfter ++ sorted(departed) == sorted(before)
...
### MEASURED AT `037a626` — ORACLES PRESENT, IMPLEMENTATION ABSENT

| arm | vector | result | departed? |
|---|---|---|---|
| SUFFIX | remove `wont_fix`, keep `completed` | **ALLOWED** | **YES** `[completed wont_fix] → [completed]` |
| SUFFIX2 | remove `duplicate`, keep `cancelled` | **ALLOWED** | **YES** `[cancelled duplicate] → [cancelled]` |
| PREFIX | remove `completed`, keep `wont_fix` | PermissionDenied | no |
| PREFIX2 | remove `cancelled`, keep `duplicate` | PermissionDenied | no |

**Two suffix pairs and two prefix pairs, so the split tracks ORDER, not stage
identity.** The prefix arms are the negative control on the characterization
itself: had all four been allowed, the mechanism would have been something other
than what I derived and the result would have been VOID.

### THE FINDING, WHICH STRENGTHENS §13 AND WAS NOT KNOWN WHEN §13 WAS WRITTEN

> **ROUND 11 DOES NOT MERELY UNDER-PRICE MASKED DEPARTURES IN SOME CORNER. IT
> PRICES AT NOTHING EVERY MASKED DEPARTURE OF A CANONICALLY FINAL STAGE, AND
> CHARGES THE REST ONLY AS A SIDE EFFECT OF SLICE ORDER.**

§13 reported the ordering accident as the thing that *saved* r11 on the vector I
happened to test. The accident **covers only the prefix half of the population**.
The suffix half was open, free, and unmeasured until now.

**AND IT MEANS MY OWN FIRST-STAGE ORACLE WAS TESTING THE COVERED HALF.** It is
green at `037a626` not because it is weak but because it picked the vector r11
denies by accident. That is why the EM refused to let me "close the gap" without
first learning which world I was in.

### RED BEFORE GREEN, AT A BASELINE WHERE THE IMPLEMENTATION IS ABSENT

- **RED** at `037a626` (`PriceLabelWrite` count 0 in `store.go`): both rows,
  message `FREE DEPARTURE`, read-back showing the stage gone.
- **GREEN** at `9cc9430`. Round 12 prices it `departed={wont_fix}`,
  `entered={}`, pair `wont_fix → completed`, matched at `any→terminal` =
  **`task:close`**, which `agentScopes()` does not hold.
- `+132 / −0`, one new test file, **no production code touched by this commit.**

### D1/D2/D3 DISPOSITION, UNCHANGED BY THIS

**D1 and D2 discharged. D3 left open and ruled unsound as written** — its premise
is measurably false, it demands PERMIT under scopes lacking `task:accept`, and it
edits *the decision* rather than the facts. `TestInsertTasksAfter_DoesNotOverDeny
StockGitHubLabels/duplicate` is deterministically RED, 5/5, **predicted in
advance**, and is the over-denial the architect must rule on.

### TRI-STATE

- **MEASURED:** the four arms above at `037a626`; RED→GREEN for the new oracle;
  `internal/server` + `internal/store` + `internal/platform/github` at `9cc9430`
  with exactly two distinct failures — D3/`duplicate` (5/5, predicted) and
  `TestWatchTasks_CreatedEvent` (**0/10 isolated, 0/5 whole-package at `9cc9430`
  AND 0/5 whole-package at `20b7525`** — a flake at rates indistinguishable at
  both endpoints, not a regression). `go vet` copylock quartet still exactly 4.
- **NOT MEASURED:** whether any *non-terminal* stage set can produce a final-suffix
  departure — the terminal path is the only one that sorts, and I did not
  enumerate the `IssueToPhaseStage` single-stage path for this property.
  CreateTask and InsertTasksAfter are untouched and unmeasured for it.
- **PRECONDITIONS:** `037a626` and `20b7525` measured in a clean `--no-local`
  clone with **no alternates**, st_dev 2049, dirty count 0. Base `2ffc22a` on
  `2cbbd92`, **not main**. Nothing pushed by me.
