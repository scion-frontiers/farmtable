...
func main() {
	...
	// Re-runs deploy-35 checks (a), (b), (c) equiv
	// ════════════════════════════════════════════════════════
	...
	fmt.Println("--- (b/a2) CreateTask with existing token ---")
	{
		...
		created, err := client.CreateTask(ctx, &pb.CreateTaskRequest{
			Name:         "__deploy-36-backward-compat-test__",
	...
	fmt.Println("--- (b.ii) CreateTask — should be REJECTED (no write scope) ---")
	{
		...
		_, err := client.CreateTask(ctx, &pb.CreateTaskRequest{
			Name:         "__deploy-36-should-be-rejected__",
			CollectionId: primaryCollectionID,
		})
	...
	{
		// GetVersion still needs IAP but no farmtable token
	...
	fmt.Println("--- (b/e2) CreateTask via dual-header ---")
	{
		ctx := dualAuthCtx(iapToken, ftToken)
		created, err := client.CreateTask(ctx, &pb.CreateTaskRequest{
			Name:         "__deploy-36-dual-header-verify__",
			CollectionId: primaryCollectionID,
		})
	...
	os.WriteFile("/scion-volumes/scratchpad/projects/farmtable/deploy/2026-07-23-deploy-36/rbac-verification-results.json", out, 0644)
